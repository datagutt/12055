...
public final class Telephony {
    ...
    public static class SIMInfo {
        ...
        public static String getOperatorByIccId(Context ctx, String iccId) {
            Log.i(TAG, "[getOperatorByIccId]+ iccId:" + iccId);
            SIMInfo simInfo = getSIMInfoByICCId(ctx, iccId);
            if(simInfo != null) {
                Log.i(TAG, "[getOperatorByIccId]- operator:" + simInfo.mOperator);
                return simInfo.mOperator;
            } else {
                Log.i(TAG, "[getOperatorByIccId]- null info, return");
                return "";
            }
        }

        public static String getOperatorBySlot(Context ctx, int nSlot) {
            Log.i(TAG, "[getOperatorBySlot]+ nSlot:" + nSlot);
            SIMInfo simInfo = getSIMInfoBySlot(ctx, nSlot);
            if(simInfo != null) {
                Log.i(TAG, "mOperator = " + simInfo.mOperator);
                return simInfo.mOperator;
            }
            else
                return "";
        ...
        public static int setOperator(Context ctx, String operator, long simId) {

            Log.i(TAG, "setDisplayNameEx SIMId " + simId + " operator = " + operator);
            if(operator == null)
                return ErrorCode.ERROR_GENERAL;
            ContentValues value = new ContentValues(1);
            value.put(SimInfo.OPERATOR, operator);
        ...
        public static int setDisplayName(Context ctx, String displayName, long SIMId) {
            ...
            Log.d("jueying","display_name:setDisplayName:"+ displayName);
            return ctx.getContentResolver().update(ContentUris.withAppendedId(SimInfo.CONTENT_URI, SIMId), 
                    value, null, null);
        }
        ...
        public static int setDisplayNameEx(Context ctx, String displayName, long SIMId, long Source) {
            ...
            ContentValues value = new ContentValues(1);
            value.put(SimInfo.DISPLAY_NAME, displayName);
            Log.d("jueying","display_name:setDisplayNameEx:"+ displayName);
