...
//#ifdef VENDOR_EDIT
    //OPPO lixuan add begin for black list at 2012-09-06
    private String blockingNumer = null;
    private Object  pollMsg = null;  
//#endif /* VENDOR_EDIT */
...
//#ifdef VENDOR_EDIT
                //OPPO lixuan modify begin for black list at 2012-09-06    
				log("handle EVENT_INCOMING_CALL_INDICATION");
                //handleIncomingCallIndication((AsyncResult)msg.obj);
                OppohandleIncomingCallIndication(msg);
//#endif /* VENDOR_EDIT */
            break;
...
//#ifdef VENDOR_EDIT
    ...
    private BroadcastReceiver  blackListReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent){			
			...
			if(getResultCode() == OppoTelephonyConstant.RESULT_BLOCK) {
                int idx = intent.getIntExtra("index", -1);
                if(idx > 0 && idx < 7){

					log("oppo block the incoming call!");
					OppohandleBlackListCall((AsyncResult)pollMsg);
                }
            } 
			else {   
               log("oppo allow the incoming call!");
			   //BaoZhu.Yu@Prd.CommApp.Telephony, bug ID 117867
			   if(null != pollMsg){
               handleIncomingCallIndication((AsyncResult)pollMsg);
			   }
            }
			   pollMsg = null;
        }
	...
	private void
	blockBlackNameList(int index,String number){
		
		Intent broadcastIntent = new Intent(OppoTelephonyConstant.ACTION_NEW_INCOMING_CALL);
		broadcastIntent.putExtra(OppoTelephonyConstant.EXTRA_INCOMING_NUMBER, number);
		broadcastIntent.putExtra("index", index);
		//BaoZhu.Yu@Prd.CommApp.Phone, 2012/12/05, Add for		
		if (FeatureOption.MTK_GEMINI_SUPPORT) {
            broadcastIntent.putExtra(PhoneConstants.GEMINI_SIM_ID_KEY, phone.getMySimId());
        }		
		phone.getContext().sendOrderedBroadcast(broadcastIntent, null, blackListReceiver, 
			null, OppoTelephonyConstant.RESULT_PASS, null, null);
    }

	private void 
	OppohandleIncomingCallIndication(Message msg){
		...
		String[] incomingCallInfo = (String[]) ar.result;
        int callId = Integer.parseInt(incomingCallInfo[0]);
        vNumber = incomingCallInfo[1];
        TOA = Integer.parseInt(incomingCallInfo[2]);
        vNumber = PhoneNumberUtils.stringFromStringAndTOA(vNumber, TOA);
		log("callId="+callId+"vNumber"+vNumber);
        blockBlackNameList(callId,vNumber);
		return;
	}
	
	private void 
	OppohandleBlackListCall(AsyncResult ar){
        int mode = 1;
        String[] incomingCallInfo = (String[]) ar.result;
        int callId = Integer.parseInt(incomingCallInfo[0]);
        int callMode = Integer.parseInt(incomingCallInfo[3]);
        int seqNumber = Integer.parseInt(incomingCallInfo[4]);
        log("mode="+mode+"callId="+callId+"seqNumber="+seqNumber+"obtainCompleteMessage="+obtainCompleteMessage());
        cm.setCallIndication(mode, callId, seqNumber, obtainCompleteMessage());
	}	
//#endif /* VENDOR_EDIT */
}
