...
import com.android.internal.telephony.PhoneFactory;
//ifdef __SMART_DUAL_SIM__
import com.android.internal.telephony.CallForwardInfo;
//endif __SMART_DUAL_SIM__
...
//#ifdef VENDOR_EDIT 
//#ifdef __SMART_DUAL_SIM__
//Huangdidan@oppo, 2012/12/14 Add for smart dual sim
    private static final int CCFC_DELAYED_TIME = 8000;
    private static boolean bQueryCCFCOnce = true;
//#endif  __SMART_DUAL_SIM__
//#endif /* VENDOR_EDIT */
    /** Notification type. */
...
//MTK-END [mtk03851][111124]MTK added
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //#ifdef VENDOR_EDIT 
            //Hong.Liu@Prd.CommApp.Telephony, 2012/02/29, Add for update plmn when statusbar crashed
            else if (intent.getAction().equals("com.oppo.request.plmnupdate")){
                log("com.oppo.request.plmnupdate");
    ...
    private ContentObserver mAutoTimeObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            ...
            //#ifndef VENDOR_EDIT
            //ChengJun.Duan@Prd.CommApp.Phone, 2013/04/15, Remove for 
            //we use NetworkTimeUpdateService to update time
            /*
            revertToNitzTime();
            */
            //#endif /* VENDOR_EDIT */
...
//MTK-START [mtk03851][111124]MTK added
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            //#ifndef VENDOR_EDIT 
            //BaoZhu.Yu@Prd.CommApp.Telephony, 2013/05/17, Add for show no service or airplaneMode when power on
        ...
        //#ifdef VENDOR_EDIT
        //Hong.Liu@Prd.CommApp.Telephony, 2012/02/29, Add for update plmn when statusbar crashed
        filter.addAction("com.oppo.request.plmnupdate");
        //#endif /* VENDOR_EDIT */
        ...
        switch (msg.what) {
            ...
            case EVENT_SET_GPRS_CONN_RETRY:
                ...
                if(FeatureOption.MTK_GEMINI_ENHANCEMENT == true){ 
                    ...
                    currentDataConnectionSimId = Settings.System.getInt(phone.getContext().getContentResolver(), GPRS_CONNECTION_SETTING, GPRS_CONNECTION_SETTING_DEFAULT) - 1; 
                } 
                Log.d(LOG_TAG, "Default Data Setting value="+currentDataConnectionSimId+" currsimId="+mSimId); 
                if(currentDataConnectionSimId != mSimId) { 
                    break; 
                } 
			...
			//ifdef __SMART_DUAL_SIM__
   			//Huangdidan@oppo, 2012/12/14 Add for smart dual sim
			case EVENT_MESSAGE_GET_CCWC:
				if (DBG) Log.d(LOG_TAG, "EVENT_MESSAGE_GET_CCWC");
				phone.getCallWaiting(obtainMessage(EVENT_MESSAGE_SEND_CCWC,
		                    							EVENT_MESSAGE_SEND_CCWC, EVENT_MESSAGE_SEND_CCWC));
				break;
			case EVENT_MESSAGE_SEND_CCWC:
				if (DBG) Log.d(LOG_TAG, "EVENT_MESSAGE_SEND_CCWC");
				ar = (AsyncResult) msg.obj;
				sendCCWCMsg(ar);
				break;
			case EVENT_MESSAGE_SEND_CCFC:
				if (DBG) Log.d(LOG_TAG, "EVENT_MESSAGE_SEND_CCFC");
				ar = (AsyncResult) msg.obj;
				sendCCFCMsg(ar);
				break;
			//#endif __SMART_DUAL_SIM__
	...
	//ifdef __SMART_DUAL_SIM__
	//Huangdidan@oppo, 2012/12/14 Add for smart dual sim
	private void sendCCWCMsg(AsyncResult ar){      
            if (ar.exception != null) {
                if (DBG) log("sendCCWCMsg: ar.exception=" + ar.exception);               
            } else {
            	if (ar.userObj instanceof Throwable) {
                	if (DBG) log("userObj == Throwable");
            	} else {
               		if (DBG) Log.d(LOG_TAG, "sendCCWCMsg: CW state successfully queried.");
               		int[] cwArray = (int[])ar.result;
               		try {
                		boolean isCallWaiting = ((cwArray[0] == 1) && ((cwArray[1] & 0x01) == 0x01));
						if(isCallWaiting){
							phone.getCallForwardingOption(CommandsInterface.CF_REASON_NOT_REACHABLE, 
								obtainMessage(EVENT_MESSAGE_SEND_CCFC, CommandsInterface.CF_REASON_NOT_REACHABLE, EVENT_MESSAGE_SEND_CCFC, null));
						}
                    } catch (ArrayIndexOutOfBoundsException e) {
                    	Log.d(LOG_TAG, "sendCCWCMsg: improper result: err =" + e.getMessage());
                    }
			   }
            }
	}
	private void sendCCFCMsg(AsyncResult ar){      
            if (ar.exception != null) {
                if (DBG) log("sendCCFCMsg: ar.exception=" + ar.exception);
            } else {
                if (ar.userObj instanceof Throwable) {
                    if (DBG) log("userObj == Throwable");
                }
                ...
                if (cfInfoArray.length == 0) {
                    if (DBG) log("sendCCFCMsg: cfInfoArray.length==0");
                } else {
                    	for (int i = 0, length = cfInfoArray.length; i < length; i++) {
                        	if (DBG) log("sendCCFCMsg, cfInfoArray[" + i + "]=" + cfInfoArray[i]);                            
                        	CallForwardInfo info = cfInfoArray[i]; 
							Intent intent = new Intent("com.oppo.ACTION_CCFC_STATE_MESSAGE");
							intent.putExtra("status", info.status);
							intent.putExtra("reason", info.reason);
							intent.putExtra("serviceClass", info.serviceClass);
							intent.putExtra("number", info.number);
							intent.putExtra("toa", info.toa);
							intent.putExtra("timeSeconds", info.timeSeconds);
    ...
    private void pollStateDone() {
        ...
        if (hasRegistered) {
            ...
            //#ifdef VENDOR_EDIT 
   			...
   			//Huangdidan@oppo, 2012/12/14 Add for smart dual sim
            if(bQueryCCFCOnce) {
                bQueryCCFCOnce = false;
                sendMessageDelayed(obtainMessage(EVENT_MESSAGE_GET_CCWC), CCFC_DELAYED_TIME);
            }
			//endif __SMART_DUAL_SIM__
			//#endif /*VENDOR_EDIT*/
        }
...
////        return 5 == code;
    ...
    private void revertToNitzTimeZone() {
        ...
        //#ifndef VENDOR_EDIT
        //ChengJun.Duan@Prd.CommApp.Phone, 2013/04/15, Modify for auto_time_zone
        /*
...
//status notification
        ...
        //#ifndef VENDOR_EDIT 
        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Remove for
...
//MTK-START [mtk03851][111124]
    ...
    protected void updateSpnDisplay(boolean forceUpdate) {
        ...
        //#ifndef VENDOR_EDIT
        //ChengJun.Duan@Prd.CommApp.Phone, 2013/03/13, Remove for don't show the string
        /*
        ...
        */
        ...
        if (imeiAbnormal == 1) {
          ...
          //#ifdef VENDOR_EDIT
          //yangyc@Prd.CommApp.Phone, 2012/08/07, modify for APP has do with 
          //don't need to add 3G after plmn
        ...
        if (showPlmn != curShowPlmn
            ...
            //#ifdef VENDOR_EDIT
            //BaoZhu.Yu@Prd.CommApp.Phone, 2012/08/07, Add for 
            showSpn  = false;
            showPlmn = true;
            log("send broadcast Intents.SPN_STRINGS_UPDATED_ACTION");
            //#endif /* VENDOR_EDIT */
            //#ifdef VENDOR_EDIT
            //WangZhi@EXP.CommService.Telephony,2013/09/17,Add for [oppo thai apn setting ]
		    if((ss.getOperatorNumeric() != null) && (ss.getOperatorNumeric().startsWith("520"))){
            ...
            //WangZhi@EXP.CommService.Telephony,2013/09/17,Add for [oppo vietnamese apn setting ]
			if((ss.getOperatorNumeric() != null) && (ss.getOperatorNumeric().startsWith("45207"))
				/*((ss.getOperatorNumeric().startsWith("45207")) || (ss.getOperatorNumeric().startsWith("45205")))*/){
		        if(plmn.equals("45207")) {
					plmn = "Beeline VN";
				}
		    	if(spn == null || spn.equals("")) {
				    showSpn  = false;
            ...
            //WangZhi@EXP.CommService.Telephony,2013/09/17,Add for [oppo indonesian apn setting ]
			if((ss.getOperatorNumeric() != null) && (ss.getOperatorNumeric().startsWith("510"))){
		    	if(spn == null || spn.equals("")) {
				    showSpn  = false;
					showPlmn = true;
				} else {
            ...
            //WangZhi@EXP.CommService.Telephony,2013/09/17,Add for [oppo russian apn setting ]
			if((ss.getOperatorNumeric() != null) && 
				((ss.getOperatorNumeric().startsWith("25001")) || (ss.getOperatorNumeric().startsWith("25002")))){
		    	if(spn == null || spn.equals("")) {
				    showSpn  = false;
					showPlmn = true;
				} else {
			...
			log("wzhi spn = " + spn + " showSpn = " + showSpn + " Numeric = " + ss.getOperatorNumeric());
            //#endif /* VENDOR_EDIT */
...
//MTK-END [mtk03851][111124]
    ...
    //#ifdef VENDOR_EDIT
    //ChengJun.Duan@Prd.CommApp.Phone, 2013/04/15, Add for auto_time_zone
    ...
    void setTimeZone(){
        ...
        if ((uniqueZones.size() == 1) || testOneUniqueOffsetPath) {
            zone = uniqueZones.get(0);
            if (DBG) {
               log("setTimeZone: no nitz but one TZ for iso-cc=" + iso +
                       " with zone.getID=" + zone.getID() +
                       " testOneUniqueOffsetPath=" + testOneUniqueOffsetPath);
            }
            setAndBroadcastNetworkSetTimeZone(zone.getID());
            ...
            if (DBG) {
                log("setTimeZone: there are " + uniqueZones.size() +
                    " unique offsets for iso-cc='" + iso +
                    " testOneUniqueOffsetPath=" + testOneUniqueOffsetPath +
                    "', do nothing");
            }
