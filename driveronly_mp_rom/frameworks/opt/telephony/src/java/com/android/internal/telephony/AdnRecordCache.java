...
public final class AdnRecordCache extends Handler implements IccConstants {
    ...
    public void oppoGetADNFieldsInfo(int efid, Message response) {
        int extensionEf=0;
        boolean isUim;        
        
        isUim = mFh.mParentApp.getType() == AppType.APPTYPE_USIM;            
        efid = oppoReviseEfIdForUsim(efid);
        if (efid == IccConstants.EF_ADN) {
            if (isUim)
                extensionEf = mUsimPhoneBookManager.oppoGetExt1EfIdForUsim();
            else 
                extensionEf = IccConstants.EF_EXT1;
        }
		
        new AdnRecordLoader(mFh).oppoGetADNFieldsInfo(efid, response,  extensionEf );
    }

    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
                                 property=android.annotation.OppoHook.OppoRomType.OPPO,
                                 note="null, only for MTK platform")
    private int oppoReviseEfIdForUsim(int efid) {
        boolean isUim;        
        
        isUim = mFh.mParentApp.getType() == AppType.APPTYPE_USIM;
        if (isUim && efid == IccConstants.EF_ADN) {
            efid= mUsimPhoneBookManager.oppoGetAdnEfIdForUsim();
        }
        return efid;
    }
    ...
    public int oppoGetTotalAdn(){
        ArrayList<AdnRecord> adns = adnLikeFiles.get(EF_ADN);
        if (adns != null){
    ...
    public int getUsimEmailLength(){
        return mUsimPhoneBookManager.getUsimEmailLength();
    }
    //#endif /* VENDOR_EDIT */
}
