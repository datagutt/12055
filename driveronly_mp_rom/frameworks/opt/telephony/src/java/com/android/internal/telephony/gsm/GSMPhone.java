...
public class GSMPhone extends PhoneBase {
    ...
    public static final String CFU_KEY = "ril.cfu.querytype";	
...
//#ifdef VENDOR_EDIT 
//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for engineer mode test
    private String emTelNumber;
    private int emTelIndex = -1;
	//bhy@Prd.CommApp.Telephony 2011/11/06 add for if get PCB failed, we can read it loop
	private boolean bPCBRead = false;
	private int nPCBReadCount = 0;
//#endif /* VENDOR_EDIT */
...
//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for if get PCB failed, we can read it loop
		sendEmptyMessageDelayed(OppoTelephonyConstant.EVENT_GET_PCB_DONE, 30000);
//#endif /* VENDOR_EDIT */
    }
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
			...
			//#ifdef VENDOR_EDIT 
			//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for factory mode read PCBA
				if (null != mSN)
				{
					try{
						//#ifdef VENDOR_EDIT 
						//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for, added for Gemini
						String fileName;
						if (FeatureOption.MTK_GEMINI_SUPPORT) {
						...
						//#endif /* VENDOR_EDIT */
				        FileOutputStream fout = mContext.openFileOutput(fileName, mContext.MODE_WORLD_READABLE);
						if (fout != null) {
...
//MTK-END [mtk80950][120410][ALPS00266631] check whether download calibration data or not
	...
	//#ifdef VENDOR_EDIT 
	//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for if get PCB failed, we can read it loop
	    case OppoTelephonyConstant.EVENT_GET_PCB_DONE:
		LOGW("HandleMessage EVENT_GET_PCB_DONE bPCBRead:" + bPCBRead);
		if (bPCBRead || (nPCBReadCount++ > 20)) {
			break;
		}
	...
	//#endif /* VENDOR_EDIT */
//#ifdef VENDOR_EDIT 
//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for engineering test  mode
            case OppoTelephonyConstant.EVENT_EM_DIAL_DONE:
                ar = (AsyncResult)msg.obj;

                if (ar.exception != null) {
            	    LOGW("OPPO failed to EVENT_EM_DIAL_DONE.");
                    break;
                }
                else{
                    LOGW("OPPO succeed to EVENT_EM_DIAL_DONE.");
                    emGetCurrentCallStatus();
                }
                break;
                
           case OppoTelephonyConstant.EVENT_EM_CALL_STATUS_CHANGED:
                LOGW("OPPO receive EVENT_EM_CALL_STATUS_CHANGED.");
                emGetCurrentCallStatus();                
                break;
                
           case OppoTelephonyConstant.EVENT_EM_GET_CURRENT_CALLS_DONE:
                ar = (AsyncResult)msg.obj;
                	
                if (ar.exception != null) {
                    LOGW("OPPO failed to EVENT_EM_GET_CURRENT_CALLS_DONE.");
                    break;
                }
                else{
                    handleEmPollsCalls(ar);
                }
                break;
                    
           case OppoTelephonyConstant.EVENT_EM_HANGUP_DONE:
                ar = (AsyncResult)msg.obj;
                	
                if (ar.exception != null) {
                    LOGW("OPPO failed to EVENT_EM_HANGUP_DONE.");
                    break;
                }
                else{
                    LOGW("OPPO succeed to EVENT_EM_HANGUP_DONE.");
                    emGetCurrentCallStatus();
                }
               	break;
               		                    
            case OppoTelephonyConstant.EVENT_SEND_UPLINK_DATA_DONE:
               	ar = (AsyncResult)msg.obj;

                if (ar.exception != null) {
                    LOGW("OPPO failed to EVENT_SEND_UPLINK_DATA_DONE.");
                    break;
                }
                    
                LOGW("Success to EVENT_SEND_UPLINK_DATA_DONE.");
                break;
//#endif /* VENDOR_EDIT */				
            default:
...
/**************************************************************/    
    ...
    public void enableEngineerTest(boolean on){
        if(LOCAL_DEBUG) Log.d(LOG_TAG, "enableEngineerTest: on = " + on);
        mCM.oppoSetEMMode(on);
        if(on) {
            mCM.oppoRegisterForEmCallStatusChanged(this, OppoTelephonyConstant.EVENT_EM_CALL_STATUS_CHANGED, null);
        }else{
            mCM.oppoUnregisterForEmCallStatusChanged(this);
        }
    }
    ...
    public void emDial(String telNumber){
    	if(!mCM.oppoGetEMMode()){
            if(LOCAL_DEBUG) Log.v(LOG_TAG, "emDial..you should call enableEngineerTest() first! ");
            return;
    	}
    	
    	if(LOCAL_DEBUG) Log.v(LOG_TAG, "emDial..telNumber: " + telNumber);
    	mCM.dial(telNumber, CommandsInterface.CLIR_DEFAULT, obtainMessage(OppoTelephonyConstant.EVENT_EM_DIAL_DONE));
    	emTelNumber = telNumber;
    }
    ...
    public void emHungup(String telNumber){
    	if(LOCAL_DEBUG) Log.v(LOG_TAG, "emHungup..telNumber: " + telNumber);

    	if(emTelNumber != null){
            //TODO here.
            if(LOCAL_DEBUG) Log.v(LOG_TAG, "emHungup..emTelNumber: " + emTelNumber);
            if(LOCAL_DEBUG) Log.v(LOG_TAG, "emHungup..emTelIndex: " + emTelIndex);
            if(emTelIndex > 0){
            	mCM.hangupConnection(emTelIndex, obtainMessage(OppoTelephonyConstant.EVENT_EM_HANGUP_DONE));
            }
    	}
    }
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
                                 property=android.annotation.OppoHook.OppoRomType.ROM,
                                 note="add for add for engineer mode test")
    private void emGetCurrentCallStatus(){
    	if(LOCAL_DEBUG) Log.v(LOG_TAG, "emGetCurrentCallStatus...");
    	mCM.getCurrentCalls(obtainMessage(OppoTelephonyConstant.EVENT_EM_GET_CURRENT_CALLS_DONE));
    }

    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
                                 property=android.annotation.OppoHook.OppoRomType.ROM,
                                 note="add for add for engineer mode test")
    private void handleEmPollsCalls(AsyncResult ar){
    	List polledCalls;
        ...
        if (ar.exception == null) {
            ...
            DriverCall dc = null;
            for (int curDC = 0, dcSize = polledCalls.size() ; curDC < dcSize; curDC++) {                	
                dc = (DriverCall) polledCalls.get(curDC);
                if(dc != null){
                    if(dc.number.equals(emTelNumber)){
                    	int emStatus;
                    	
                    	emTelIndex = curDC+1;
                    	emStatus = getEmCallStatus(dc.state);
                    	if(LOCAL_DEBUG) Log.v(LOG_TAG, "emStatus: " + emStatus);
                    	
                    	//TODO. send this status up to mmi.
                    	notifyEMTCallStatus(emStatus, emTelNumber);
                    	return;
                    }
                }
            }
            
            if(LOCAL_DEBUG) Log.v(LOG_TAG, "tel: " + emTelNumber + " hung up.");
            //notify mmi current call has hungup.
            notifyEMTCallStatus(OppoTelephonyConstant.EMT_CALL_STATE_IDLE, emTelNumber);
        }//end if
        else{
            if(LOCAL_DEBUG) Log.v(LOG_TAG, "handleEmPollsCalls error.");
            notifyEMTCallStatus(OppoTelephonyConstant.EMT_CALL_STATE_ERROR, emTelNumber);
    	}
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
                                 property=android.annotation.OppoHook.OppoRomType.ROM,
                                 note="add for add for engineer mode test")
    private int getEmCallStatus(DriverCall.State state)
    {//see TelephonyManager.java 
        switch (state) {
            case ACTIVE:
            	return OppoTelephonyConstant.EMT_CALL_STATE_ACTIVE;
            case DIALING:
                return OppoTelephonyConstant.EMT_CALL_STATE_DIALING;
            case ALERTING:
                return OppoTelephonyConstant.EMT_CALL_STATE_ALERTING;
            case HOLDING:
                return OppoTelephonyConstant.EMT_CALL_STATE_HOLDING;
            default:
                return OppoTelephonyConstant.EMT_CALL_STATE_UNKNOWN;
        }
    }
    
    /**
     * oppo shenxianshu add
     * status: see TelephonyManager.java 
     */
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
                                 property=android.annotation.OppoHook.OppoRomType.ROM,
                                 note="add for add for engineer mode test")
    private void notifyEMTCallStatus(int status, String incomingNumber){
    	if(LOCAL_DEBUG) Log.v(LOG_TAG, "notifyEMTCallStatus: status = " + status + " number:" + incomingNumber);
    	Intent intent = new Intent(OppoTelephonyConstant.ACTION_EMT_CALL_STATE_CHANGED);
    	intent.putExtra(OppoTelephonyConstant.EXTRA_EMT_CALL_STATE, status);
    	intent.putExtra(OppoTelephonyConstant.EXTRA_EMT_CALL_NUMBER, incomingNumber);
		//#ifdef VENDOR_EDIT 
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for , added for Gemini
		if(FeatureOption.MTK_GEMINI_SUPPORT){
			intent.putExtra(PhoneConstants.GEMINI_SIM_ID_KEY,getMySimId());
		}
		//#endif /* VENDOR_EDIT */
