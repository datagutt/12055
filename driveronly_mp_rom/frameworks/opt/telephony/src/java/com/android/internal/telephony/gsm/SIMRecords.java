...
//MTK-START [mtk80601][111215][ALPS00093395]
    ...
    /*MTK proprietary end */
    //#ifdef VENDOR_EDIT
    //Hong.Liu@Prd.CommApp.Telephony, 2012/05/08,Add for
    private int pooledTime = 0; 
    private String oppoMsisdnNumber = null;
    private String oppoMsisdnTag = null;
    //#endif /* VENDOR_EDIT */
...
//MTK-END [mtk80601][111215][ALPS00093395]
    ...
    public SIMRecords(UiccCardApplication app, Context c, CommandsInterface ci) {
        ...
        //#ifdef VENDOR_EDIT 
        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/08/16,Add for start phb poll        
        ...
        //#ifdef VENDOR_EDIT 
        //Hong.Liu@Prd.CommApp.Telephony, 2012/05/08,Add for start phb poll
        if(mSimId == PhoneConstants.GEMINI_SIM_1){
            sendMessageDelayed(obtainMessage(OppoTelephonyConstant.EVENT_POLL_IS_PHB_READY), 
                OppoTelephonyConstant.PHB_START_POLL_TIME1);
        }else{
            sendMessageDelayed(obtainMessage(OppoTelephonyConstant.EVENT_POLL_IS_PHB_READY), 
                OppoTelephonyConstant.PHB_START_POLL_TIME2);    
        }
        //#endif /* VENDOR_EDIT */
        try{
    ...
    protected void resetRecords() {
        ...
        if (!mCi.getRadioState().isAvailable()) {
            ...
            //#ifndef VENDOR_EDIT 
            //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Modify for
            //adnCache.reset();
            //#else /* VENDOR_EDIT */
            adnCache.reset();
            //#endif /* VENDOR_EDIT */
    ...
    public void setMsisdnNumber(String alphaTag, String number,
        ...
        */		
        //#else /* VENDOR_EDIT */
        oppoMsisdnNumber = number;
        oppoMsisdnTag = alphaTag;
		
        if(DBG) log("Set MSISDN: " + oppoMsisdnTag +" " + oppoMsisdnNumber);

        AdnRecord adn = new AdnRecord(oppoMsisdnTag, oppoMsisdnNumber);
        //#endif /* VENDOR_EDIT */
...
//MTK-END [mtk80601][111215][ALPS00093395]
            ...
            case EVENT_GET_IMSI_DONE:
                ...
                if (ar.exception != null) {
                    ...
                    //#ifdef VENDOR_EDIT 
                    //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for SIM IO error for some sim cards */	
                    Log.e(LOG_TAG, "IMSI Exception, so retry getImsi");
                    fetchSimRecords();
                    //#endif /* VENDOR_EDIT */                
                    break;
                }
                ...
                //#ifdef VENDOR_EDIT
                //WangZhi@EXP.CommService.Telephony,2013/09/17,Add for [solve the issue,which can't fetch spn from dtac SIM card]
				if ((mImsi != null) && ((mImsi.startsWith("52018")) || (mImsi.startsWith("52005")))){
					spn = "DTAC";
					if (mSimId == PhoneConstants.GEMINI_SIM_1) {
						SystemProperties.set(PROPERTY_ICC_OPERATOR_ALPHA, spn);
						SystemProperties.set(PROPERTY_ICC_OPERATOR_DEFAULT_NAME, spn);						
					} else {
					    SystemProperties.set(PROPERTY_ICC_OPERATOR_ALPHA_2, spn);
					    SystemProperties.set(PROPERTY_ICC_OPERATOR_DEFAULT_NAME_2, spn);
					}
					log("lala DTAC pass");
				}
    			//#endif /* VENDOR_EDIT */   
            ...
            case EVENT_GET_MSISDN_DONE:
                ...
                if (ar.exception != null) {
                    //#ifdef VENDOR_EDIT 
                    //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
                    int simrecordCount = Integer.parseInt(ar.userObj.toString()); 

                    if(simrecordCount < OppoTelephonyConstant.MAXNUM_RETRY_SIMRECORD) {
                        isRecordLoadResponse = false;
                        simrecordCount++;
                        new AdnRecordLoader(mFh).loadFromEF(IccConstants.EF_MSISDN, IccConstants.EF_EXT1, 1,
                            obtainMessage(EVENT_GET_MSISDN_DONE, simrecordCount));
                    }
                    //#endif /* VENDOR_EDIT */     
            ...
            case EVENT_SET_MSISDN_DONE:
                ...
                //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
                if (ar.exception != null) {
                    Log.d(LOG_TAG, "[EVENT_SET_MSISDN_DONE] fail.");
                } else {
                    Log.d(LOG_TAG, "[EVENT_SET_MSISDN_DONE] success!");
                    msisdn = oppoMsisdnNumber;
                    msisdnTag = oppoMsisdnTag;
                    setNumberForNewSIM();
                }
                //#endif /* VENDOR_EDIT */                
                if (ar.userObj != null) {
...
//MTK-END [mtk80601][111215][ALPS00093395]
            ...
            case EVENT_GET_SST_DONE:
                ...
                if (ar.exception != null) {
                    ...
                    Log.d(LOG_TAG, "EVENT_GET_SST_DONE error msg.arg1 is "+ simrecordCount);
                    if(simrecordCount < OppoTelephonyConstant.MAXNUM_RETRY_SIMRECORD) {
                        isRecordLoadResponse = false;
                        simrecordCount++;
                        mFh.loadEFTransparent(IccConstants.EF_SST, obtainMessage(EVENT_GET_SST_DONE, simrecordCount));
                    }
                    //#endif /* VENDOR_EDIT */      
                    break;
                }
...
//MTK-END [mtk80601][111215][ALPS00093395]
            ...
            */
            ...
            //#ifdef VENDOR_EDIT
            //Hong.Liu@Prd.CommApp.Telephony, 2012/05/08,Add for
            case OppoTelephonyConstant.EVENT_POLL_IS_PHB_READY:
                boolean pollGoon = false;
                String property = null;

                if(mSimId == PhoneConstants.GEMINI_SIM_1){
                    property = "oppo.contacts.sim1ready";
                }else if(mSimId == PhoneConstants.GEMINI_SIM_2){
                    property = "oppo.contacts.sim2ready";
                }
                
                if(SystemProperties.getBoolean(property, false) == false 
                        && mPhone.getServiceState().getState() != ServiceState.STATE_POWER_OFF 
                        && pooledTime < OppoTelephonyConstant.POLL_MAX_TIME){
                    Intent intent = new Intent("android.intent.action.oppo_check_simphb_ready");
                    mContext.sendBroadcast(intent);
                    pollGoon = true;
                    pooledTime++;
                }                    

                if (DBG) log("EVENT_POLL_IS_PHB_READY property ="+property
                                +" getServiceState="+mPhone.getServiceState().getState()
                                +" pooledTime="+pooledTime
                                +" "+SystemProperties.getBoolean(property, false));
                
                if(pollGoon){
                    sendMessageDelayed(obtainMessage(OppoTelephonyConstant.EVENT_POLL_IS_PHB_READY), 
                        OppoTelephonyConstant.PHB_POLL_TIMEOUT);
                }
                break;    
            //#endif /* VENDOR_EDIT */
            default:
...
////        phone.setNetworkSelectionModeAutomatic(null);
    ...
    private void setNumberForNewSIM(){
        ...
        if (simInfo!= null) {			
            ...
            //#ifndef VENDOR_EDIT 
            //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Remove for reset simInfo.Number
            //if (simNumber == null ) 
            //#endif /* VENDOR_EDIT */
            {
    ...
    protected void onAllRecordsLoaded() {
        ...
        if(operator != null) {
            ...
            */
            ...
            //#ifndef VENDOR_EDIT
            //BaoZhu.Yu@Prd.CommApp.Phone, 2012/08/30, Add for 
            /*
...
//MTK-START [mtk80601][111215][ALPS00093395]
    private void fetchPhbRecords() {
		...
		*/
        //#else /* VENDOR_EDIT */ 
         new AdnRecordLoader(mFh).loadFromEF(EF_MSISDN, EF_EXT1, 1,
                        obtainMessage(EVENT_GET_MSISDN_DONE,0));
        //#endif /*VENDOR_EDIT*/

        //#ifdef VENDOR_EDIT 
        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for, modify for phoneapp recreate need LoadAllAdnLike,
        mPhone.getIccPhoneBookInterfaceManager().onPhbReady();
        //#endif /* VENDOR_EDIT */
    }
...
//MTK-END [mtk80601][111215][ALPS00093395]
    protected void fetchSimRecords() {
        ...
        //#else /* VENDOR_EDIT */ 
        mFh.loadEFTransparent(EF_SST, obtainMessage(EVENT_GET_SST_DONE,0));
        //#endif /*VENDOR_EDIT*/
        recordsToLoad++;
            ...
            if (action.equals("com.mediatek.dm.LAWMO_WIPE")) {
               wipeAllSIMContacts();
                ...
                //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/08/16,Add for start phb poll
                    ...
                    Log.d(LOG_TAG, "ACTION_PREBOOT_IPO");
                    if(mSimId == PhoneConstants.GEMINI_SIM_1){
                        sendMessageDelayed(obtainMessage(OppoTelephonyConstant.EVENT_POLL_IS_PHB_READY), 
                            OppoTelephonyConstant.PHB_START_POLL_TIME1);
                    } else {
        ...
        if(numeric != null) {
            ...
            if (simInfo!= null) {
                if(numeric.equals("46001")) {
                    SIMInfo.setOperator(mContext,SimInfo.OPERATOR_OP02,simInfo.mSimId);
                } else if(numeric.equals("46000") || numeric.equals("46002") || numeric.equals("46007")) {
                    SIMInfo.setOperator(mContext,SimInfo.OPERATOR_OP01,simInfo.mSimId);
                } else {
                    SIMInfo.setOperator(mContext,SimInfo.OPERATOR_OTHERS,simInfo.mSimId);                
                }
            }
        } else {
