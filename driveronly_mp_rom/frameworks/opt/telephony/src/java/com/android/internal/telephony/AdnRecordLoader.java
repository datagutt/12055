...
public class AdnRecordLoader extends Handler {
    ...
    public void loadAllFromEF(int ef, int extensionEF,
        ...
        //#ifndef VENDOR_EDIT 
        // #@OppoHook
        //opporom@Prd.CommApp.Telephony, 2012/08/16, Modify for
        if (type != -1) {
            mFh.mCi.queryPhbStorageInfo(
                    type,
                    obtainMessage(EVENT_PHB_QUERY_STAUTS,0));
        } else {
            mFh.loadEFLinearFixedAll(
                    ef,
                    obtainMessage(EVENT_ADN_LOAD_ALL_DONE,0));
        }
        //#endif /* VENDOR_EDIT */
    }
            ...
            switch (msg.what) {
                ...
                case EVENT_ADN_LOAD_ALL_DONE:
                    ...
                    if (ar.exception != null) {
                        ...
                        int retryCount = Integer.parseInt(ar.userObj.toString());
                        if(retryCount < OppoTelephonyConstant.MAXNUM_RETRY_SIMRECORD) {
                            retryCount++;
                            mFh.loadEFLinearFixedAll(ef,obtainMessage(EVENT_ADN_LOAD_ALL_DONE, retryCount));                            
                            break;
                        }else
                        //#endif /* VENDOR_EDIT */
                        throw new RuntimeException("load failed", ar.exception);
                ...
                //#ifdef VENDOR_EDIT 
                //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
                case OppoTelephonyConstant.OPPO_EVENT_ADN_SIZE_DONE:
                    ar = (AsyncResult)(msg.obj);
                    ...
                    ret[1] =  resultInfo[0] - AdnRecord.FOOTER_SIZE_BYTES; // adn record size = alphaTab leng + 14
                    ret[0] = 20;
                ...
                case EVENT_PHB_QUERY_STAUTS:
                    ...
                    if (ar.exception != null) {
                        //#ifdef VENDOR_EDIT 
                        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/08/16, Add for
                        int retryCount = Integer.parseInt(ar.userObj.toString());
                        if(retryCount < OppoTelephonyConstant.MAXNUM_RETRY_SIMRECORD) {
                            retryCount++;
                            type = getPhbStorageType(ef);
                            mFh.mCi.queryPhbStorageInfo(type,obtainMessage(EVENT_PHB_QUERY_STAUTS,retryCount));   
                            break;
                        }else
                        //#endif /* VENDOR_EDIT */
                ...
                case EVENT_PHB_LOAD_ALL_DONE:
                    ...
                    if (ar.exception != null) {
                        //#ifdef VENDOR_EDIT 
                        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
                        if(readInfo[3] < OppoTelephonyConstant.MAXNUM_RETRY_SIMRECORD) {
                            readInfo[3]++;
                            type = getPhbStorageType(ef);
                            readEntryFromModem(type, readInfo);
                            break;
    ...
    private void writeEntryToModem(AdnRecord adn, int type) {
        ...
        //#else /* VENDOR_EDIT */
        //bhy@oppo.com add 2011/10/17 for remove PhoneNumberUtils.OPPO_WAIT with MTK modem not support 'w'
        ...
        if (!(number.equals("") && alphaId.equals("") && ton == 0x81)) {
            ...
            //#ifdef VENDOR_EDIT 
            //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for remove not support charactor
    ...
    private AdnRecord getAdnRecordFromPhbEntry(PhbEntry entry) {
        ...
        number = number.replace('?', PhoneNumberUtils.WILD);
        //#ifndef VENDOR_EDIT 
        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Remove for delete 20110824 for phonebook 
        // replace P with ,
    ...
    public void oppoGetADNFieldsInfo(int efid, Message response, int ext1Ef) {
        this.ef = efid;
        this.userResponse = response;
        mFh.getEFLinearRecordSize( efid, obtainMessage(OppoTelephonyConstant.OPPO_EVENT_ADN_SIZE_DONE, ext1Ef, 0));
    }
    //#endif /* VENDOR_EDIT */	
}
