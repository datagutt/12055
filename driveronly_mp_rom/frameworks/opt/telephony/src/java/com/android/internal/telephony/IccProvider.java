...
public class IccProvider extends ContentProvider {
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (UPB == match || UPB1 == match || UPB2 == match) {
            ...
            if (ADDRESS_BOOK_COLUMN_NAMES.length >= ADDRESS_SUPPORT_AAS) {
                ...
                if (result < 0){
                    if (DBG) log("oppoAddUsimRecordToEf fail.");
                    return null;
                }
                //#endif /* VENDOR_EDIT */
            }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        //#@OppoHook
        //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
        //liuhong@oppo.com modified for Gemini,2011.12.26
        if (!TextUtils.isEmpty(id) && (match == UPB || match == UPB1 || match == UPB2)) {
			return deleteUsimRecordFromEfByIndex(efType, Integer.valueOf(id), simId);                
        } else if (!TextUtils.isEmpty(id) && (match == ADN || match == ADN1 || match == ADN2)) {
        	return deleteIccRecordFromEfByIndex(efType, Integer.valueOf(id), pin2, simId);
        }
        //#endif /* VENDOR_EDIT */
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
        ...
        if (DBG) log("[update] tag: " + tag + " number: " + number + " newTag: " + newTag + " newNumber: " + newNumber + " id: " + id);
        if(newTag == null) {
            newTag = "";
        }
        if(newNumber == null) {
        ...
        if (id != null) {
			//liuhong@oppo.com modified for gemini,2011.12.26
			if (match == UPB || match == UPB1 || match == UPB2){
				...
				return updateUsimRecordInEfByIndex(efType, id.intValue(), newTag,  newNumber, strAnr, strEmail, simId);
			} else if (match == ADN || match == ADN1 || match == ADN2){
				return updateIccRecordInEfByIndex(efType, id.intValue(), newTag, newNumber,pin2, simId);
			}
        ...
        if (!record.isEmpty()) {
            ...
            if (emails != null) {
                ...
                for (String email : emails) {
                    ...
                    //#ifdef VENDOR_EDIT
                    //#@OppoHook
                    //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/10/09, Add for 
                    ...
                    //#ifndef VENDOR_EDIT
                    //#@OppoHook
                    //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/10/09, Remove for
                    //emailString.append(",");
                    //#endif /* VENDOR_EDIT*/
                }
