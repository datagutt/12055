...
final class WiredAccessoryManager implements WiredAccessoryCallbacks {
    ...
    private void updateLocked(String newName, int newState) {
        ...
        int usb_headset = newOrOld & (BIT_USB_HEADSET_ANLG | BIT_USB_HEADSET_DGTL);
        int usb_state   = (headsetState & (BIT_USB_HEADSET_ANLG | BIT_USB_HEADSET_DGTL));
        int usb_mstate  = (mHeadsetState & (BIT_USB_HEADSET_ANLG | BIT_USB_HEADSET_DGTL));
		int hdmi = newOrOld & (BIT_HDMI_AUDIO);
        int hdmi_state   = (headsetState & BIT_HDMI_AUDIO);
        int hdmi_mstate  = (mHeadsetState & BIT_HDMI_AUDIO);
		...
		if (h2w_state == BIT_HEADSET_NO_MIC && h2w_mstate == BIT_HEADSET) {
...
//		//showheadsetToast();
        ...
        if (usb_mstate == usb_state || ((usb_headset & (usb_headset - 1)) != 0)) {
            Log.e(TAG, "Invalid combination, unsetting usb flag");
            usbStateChange = false;
        }
		if (hdmi_state == hdmi_mstate) {
            Log.e(TAG, "unsetting hdmi flag");
