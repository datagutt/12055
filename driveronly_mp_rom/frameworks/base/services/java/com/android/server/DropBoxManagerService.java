...
//#ifdef VENDOR_EDIT
//#TongXi.Li@Plf.Framework add for oppo_assert feed_back
...
public final class DropBoxManagerService extends IDropBoxManagerService.Stub {
    ...
    private void sendFeedbackBroadcast(ArrayList<String> files) {
		...
		try {
			pakageName = new OppoActivityManager().getTopActivityComponentName().getPackageName();
		} catch (Exception e) {
			e.printStackTrace();
		}
 	...
 	@android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
								property=android.annotation.OppoHook.OppoRomType.ROM,
            					note="TongXi.Li@Plf.Framework modify for use oppo_assert ")
    @Override
    public void add(DropBoxManager.Entry entry) {
        ...
        //#ifdef VENDOR_EDIT  
        //#@OppoHook
        //TongXi.Li@Plf.Framework, 2013/1/25, add for oppo_assert feed_back
		final boolean assertEnable = (SystemProperties.getBoolean("persist.sys.assert.enable", false)
        ...
        try {
            ...
            //#ifndef VENDOR_EDIT
        	//#@OppoHook
			//#TongXi.Li@Plf.Framework modify for oppo_assert feed_back
            /*
