...
//#ifdef VENDOR_EDIT
//#@OppoHook
//ZhiYong.Lin@Plf.Framework, Add for secure protect 2013/01/08
...
//#ifdef VENDOR_EDIT
//Zhanhong.Hu@Prd.SystemSrv.AMS, 2012/03/26, Add for activity proctected.
    private ActivityRecord resultToTemp;
    private String resultWhoTemp;
    private int requestCodeTemp;
    private ActivityRecord sourceRecordTemp = null;       
	private int originalIntentFlag = 0x0;
	private boolean bRecordUsed = false;
//#endif /* VENDOR_EDIT */
    ...
    OppoActivityStackHelper mOppoActivityStackHelper = new OppoActivityStackHelper();
    //#endif VENDOR_EDIT
    ...
    final ActivityRecord topRunningActivityLocked(ActivityRecord notTop) {
        ...
        while (i >= 0) {
			...
			//#ifndef VENDOR_EDIT
        	//#@OppoHook
			//Tongxi.Li@Plf.Framework, 2013/08/26, modify for MTK patch for resolve system_server be killed when enable UMS oppo_usb
        	...
        	//#else /* VENDOR_EDIT */
			if (!r.finishing && r != notTop && okToShow(r) && !r.willBeFinished) {
        	/// @}
        	//#endif /* VENDOR_EDIT */            
                return r;
            }
...
//#ifdef VENDOR_EDIT 
//Zhanhong.Hu@Prd.SystemSrv.AMS, 2012/12/24, Add for MTK IPO
				if (mService.skipList != null && mService.skipList.contains(mResumedActivity.packageName)) {
					Slog.v(TAG, "DEBUG INFO : SKIP SLEEP " + mResumedActivity.packageName);
					return;
				}
//#endif /* VENDOR_EDIT */				
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        android.oppoutils.OppoUtils.cpuBoost(1500000);
		//#endif /* VENDOR_EDIT */
        ...
        if (next == null) {
            ...
            if (mMainStack) {
				...
				mOppoActivityStackHelper.sendIsHomeModeIntent(mContext, true);
				// #endif VENDOR_EDIT
		...
		//#@OppoHook
		//ZhiYong.Lin@Plf.Framework, Add for statusbar tansparent 2013/01/08
		mOppoActivityStackHelper.handleSendHomeMode(mContext, prev, next);
		//#endif VENDOR_EDIT
		
		//#ifdef VENDOR_EDIT
		//#@OppoHook
		//ZhiYong.Lin@Plf.Framework, Add for three finger screen capture 2013/02/17
		mOppoActivityStackHelper.handleSendCameraMode(mContext, prev, next);
		//#endif VENDOR_EDIT
        ...
        //#ifdef VENDOR_EDIT
        //Yadong.Hu@Prd.BasicSrv.StatusBar, 2013/03/08, Add for change the values of alpha for StatusBar
		if (prev != null) {
            if ((next.isHomeActivity) && (!prev.isHomeActivity)) {
                Slog.i(TAG, "(test)resumeTopActivityLocked: change to homeMode");
                final Intent intent = new Intent("Intent.ACTION_HOME_MODE_CHANGE");
                intent.putExtra("ishomemode", true);
                mContext.sendBroadcast(intent);
            } else if ((!next.isHomeActivity) && (prev.isHomeActivity)){
                Slog.i(TAG, "(test)resumeTopActivityLocked:  statusbar change to notHomeMode");
                final Intent intent = new Intent("Intent.ACTION_HOME_MODE_CHANGE");
                intent.putExtra("ishomemode", false);
                mContext.sendBroadcast(intent);
            }    
        } else {
            if (next.isHomeActivity) {
                Slog.i(TAG, "(test)resumeTopActivityLocked:prev is null change to homeMode");
                final Intent intent = new Intent("Intent.ACTION_HOME_MODE_CHANGE");
                intent.putExtra("ishomemode", true);
                mContext.sendBroadcast(intent);    
            }
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
            ...
            Slog.e(TAG, "START {" + intent.toShortString(true, true, true, false)
            ...
            if (mService.mController != null) {
                ...
                if (abort) {
                    if (resultRecord != null) {
                        ...
                        */
                        //#else VENDOR_EDIT
                        mOppoSecureProtectUtils.setTempValue(resultRecord, resultWho, requestCode);
                        //#endif VENDOR_EDIT
                    }
		...
		*/
		//#else VENDOR_EDIT
        ActivityRecord r = mOppoSecureProtectUtils.handleStartActivityLocked(mService, this, 
        		callerApp, callingUid, intent, resolvedType, aInfo, resultRecord, resultWho, 
    ...
    final int startActivityUncheckedLocked(ActivityRecord r,
		...
		*/
		//#else VENDOR_EDIT
        if (r.resultTo != null && (launchFlags&Intent.FLAG_ACTIVITY_NEW_TASK) != 0 
        		&& (launchFlags&OppoIntent.OPPO_FLAG_ACTIVITY_SECURE_POLICY) == 0) {
        //#endif VENDOR_EDIT
        ...
        if (((launchFlags&Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
            ...
            if (r.resultTo == null) {
                ...
                if (taskTop != null) {
					...
					*/
					//#else VENDOR_EDIT
                    if ((!addingToTask && reuseTask == null) && (r.packageName != null && !r.packageName.equals("com.android.secure")) && 
                    		(launchFlags&OppoIntent.OPPO_FLAG_ACTIVITY_SECURE_POLICY) == 0) {
                    //#endif VENDOR_EDIT
...
//#ifdef VENDOR_EDIT
//Zhanhong.Hu@SystemSrv.AMS, 2013/02/22, Add for Remove MTK ipoWindow,[MTK IPO]
        if (mService.mIPOWin != null && token != null) {
            int index = indexOfTokenLocked(token);
            ActivityRecord r = (ActivityRecord)mHistory.get(index);
            Slog.v(TAG, "name = " + r.realActivity.getClassName());
			if (r.realActivity.getClassName().equals(android.app.AlarmManager.ALARM_BOOT_WINDOW)) {
                Slog.v(TAG, "reset mAlarmBoot & remove IPOWin");
                mService.removeIPOWin(mContext);
            }
        }
//#endif /* VENDOR_EDIT */
    ...
    final boolean finishActivityLocked(ActivityRecord r, int index, int resultCode,
    	...
    	//#@OppoHook
		//ZhiYong.Lin@Plf.Framework, Add for secure protect 2013/01/08
    	mOppoSecureProtectUtils.handleFinishActivityLocked(this);
    	//#endif VENDOR_EDIT
	...
	final ActivityRecord getTopRunningActivityLocked(ActivityRecord notTop) {
		synchronized (mService) {
			return topRunningActivityLocked(notTop);
		}
	}
