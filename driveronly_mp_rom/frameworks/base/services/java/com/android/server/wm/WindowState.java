...
final class WindowState implements WindowManagerPolicy.WindowState {
    ...
    boolean showLw(boolean doAnimation, boolean requestAnim) {
		...
		if (mAttrs.type == WindowManager.LayoutParams.TYPE_APK_LOCKSCREEN) {
			requestAnim = false;
		
			final int N = mChildWindows.size();
			for (int i=0; i<N; i++) {
				mChildWindows.get(i).showLw(doAnimation, requestAnim);
			}
		}
		//#endif /* VENDOR_EDIT */
    ...
    boolean hideLw(boolean doAnimation, boolean requestAnim) {
		...
		if (mAttrs.type == WindowManager.LayoutParams.TYPE_APK_LOCKSCREEN) {		
			...
			final int N = mChildWindows.size();
            for (int i=0; i<N; i++) {
                mChildWindows.get(i).hideLw(doAnimation, requestAnim);
            }
		}	
		//#endif /* VENDOR_EDIT */
