...
//#ifdef VENDOR_EDIT
//yaolang@Plf.DevSrv.Storage, 2012/06/25, Modify for
import java.util.Arrays;
import com.oppo.os.OppoEnvironment;

//TongXi.Li@Plf.DevSrv.Storage, 2012/10/29, Add for resolve bug: 101770
...
//#ifndef VENDOR_EDIT
//LinJie.Xu@Prd.SysSrv.USB, 2013/03/08, Modify for oppo dynmicmount
/*
    private static final String                   EXTERNAL_SD2 = "/storage/sdcard1";
*/
//#else /* VENDOR_EDIT */
    private static final String                   EXTERNAL_SD2 = (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT) ? "/storage/sdcard0/external_sd" : "/storage/sdcard1";
//#endif /* VENDOR_EDIT */
    // by MTP request
    ...
    class UnmountCallBack {
        ...
        void handleFinished() {
			...
			//#ifndef VENDOR_EDIT
			//LinJie.Xu@Prd.SysSrv.USB, 2012/07/17, Modify for when dynamic mount SD,and sd exit.when unmount sd should unmount emmc first;
			/*
				...
				*/
			//#else /* VENDOR_EDIT */
				if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT 
					&& isSDExist()
					&& path.equals(OppoEnvironment.PARENT_STORAGE_DIRECTORY.toString()))
				{
						unMountAll(removeEncryption);
				}
				else
				{
					 doUnmountVolume(path, true, removeEncryption);
    ...
    class UmsEnableCallBack extends UnmountCallBack {
        ...
        void handleFinished() {
            ...
            //#ifndef VENDOR_EDIT
            //LinJie.Xu@Prd.SysSrv.USB, 2012/07/10, Modify for 
            /*
		        if (!mIsUsbConnected) {
        ...
        */
        //#else /* VENDOR_EDIT */
            ...
            if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT 
                ...
                && path.equals(OppoEnvironment.PARENT_STORAGE_DIRECTORY.toString()))
            {
                	ret = unMountAll(removeEncryption);
            }
            else
            {
                    ret = doUnmountVolume(path, true, removeEncryption);
            }
             Slog.i(TAG, "ShutdownCallBack ret = " + ret);
        //#endif /* VENDOR_EDIT */
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case H_UNMOUNT_PM_UPDATE: {
                    ...
                    //#ifndef VENDOR_EDIT
                    //LinJie.Xu@Prd.SysSrv.USB, 2013/03/26, Modify for 
                    /*
                    ...
                    */
                    //#else /* VENDOR_EDIT */
                        ...
                        if (ucb.path.equals(EXTERNAL_SD1) &&(isSDExist() || !FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT)) {
                            mUpdatingStatus = true;
                             if (DEBUG_UNMOUNT) Slog.i(TAG, "need update external media status on PackageManager"); 
                            mPms.updateExternalMediaStatus(false, true);
                        } else {
                            //if shutdown or turn on UMS, and mnt/sdcard need unmount
                            //do not send message until PMS done
                             if (DEBUG_UNMOUNT) Slog.i(TAG, "finishMediaUpdate"); 
                            finishMediaUpdate();
                        }
                    //#endif /* VENDOR_EDIT */
    ...
    private void handleSystemReady() {
        ...
        if (FeatureOption.MTK_2SDCARD_SWAP) {
            paths = rawPaths;
            //#ifdef VENDOR_EDIT
            //LinJie.Xu@Prd.SysSrv.USB, 2013/01/22, Add for oppo dynamic mount[storage/sdcard0, storage/sdcard0/external_sd]
               if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT)
               {
                 Arrays.sort(paths);
               }
    ...
    private final class MountServiceBinderListener implements IBinder.DeathRecipient {
        ...
        //#ifdef VENDOR_EDIT
        //OppoHook
		//ZhiYong.Lin@Plf.Framework modify for BPM 2012/01/21
        int listenerPid;
        //#endif VENDOR_EDIT 
        ...
        MountServiceBinderListener(IMountServiceListener listener) {
            ...
            //OppoHook
			//ZhiYong.Lin@Plf.Framework modify for BPM 2012/01/21
            listenerPid = Binder.getCallingPid();
            //#endif VENDOR_EDIT
        }
        ...
        synchronized (mListeners) {
            for (int i = mListeners.size() -1; i >= 0; i--) {
                ...
                try {
                    ...
                    //OppoHook
					//ZhiYong.Lin@Plf.Framework modify for BPM 2012/01/21
                    android.os.Process.sendSignal(bl.listenerPid, 18);
                    //#endif VENDOR_EDIT
    ...
    //#ifndef VENDOR_EDIT
    //LinJie.Xu@Prd.SysSrv.USB, 2013/03/26, Remove for wo do mount in USBService
    /*
    ...
    private void readStorageListLocked() {
        ...
        mVolumeStates.clear();
        //#ifdef VENDOR_EDIT
        //LinJie.Xu@Prd.SysSrv.USB, 2013/02/23, Add for 
                ...
                if (TAG_STORAGE.equals(element)) {
                    ...
                    //LinJie.Xu@Prd.SysSrv.USB, 2013/01/22, Add for oppo dynamic mount[storage/sdcard0, storage/sdcard0/external_sd]
                       if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT)
                       {
                         ...
                         { 
                            Slog.d(TAG, "oppo dynamic mount,need change /storage/sdcard1 to /storage/sdcard0/external_sd");
    ...
    public void shutdown(final IMountShutdownObserver observer) {
        ...
        synchronized (mVolumesLock) {
        	...
        	//#ifndef VENDOR_EDIT
            //TongXi.Li@Plf.DevSrv.Storage, 2012/10/30, Modify for fix bug:103407
            ...
            for(int i = 0; i < count; i++){
                String path = paths[i];
                String state = mVolumeStates.get(paths[i]);
            //#endif /* VENDOR_EDIT */
                ...
                //#ifdef VENDOR_EDIT
                //LinJie.Xu@Prd.SysSrv.USB, 2013/03/26, Add for  if we mounted as /storage/sdcard0 /storage/sdcard0/external_sd
                // we just unmount /storage/sdcard0 ,because we will unmount all in handle finish. 
                    if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT)
                    {
                        break;
                    }
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (enable && state.equals(Environment.MEDIA_MOUNTED)) {
                ...
                setUmsEnabling(false);
                     //#ifdef VENDOR_EDIT
                    //LinJie.Xu@Prd.SysSrv.USB, 2012/07/04, Add for 
                    /**
                    * we should mount /mnt/sdcard first,then mount /mnt/sdcard/external_sd
                    * and unmount /mnt/sdcard/external_sd first then unmount /mnt/sdcard
                    */
                    if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT)
                    {
                        break;
	...
	//#ifdef VENDOR_EDIT
	//LinJie.Xu@Prd.SysSrv.USB, 2012/07/17, Add for 
	private void shareUnshareAllVolume(boolean enable) {		
		  Slog.d(TAG, "shareUnshareAllVolume "+enable);    
		  String method = "ums";		
		  setUmsEnabling(enable);
			 String[] paths;
			int count;
	...
	private int unMountAll(boolean removeEncryption)
	{
		int rc = StorageResultCode.OperationSucceeded;
			String[] paths;
			int count;
		 synchronized (mVolumeStates) {
			  ...
			  for (int i = count -1; i >= 0; i--) {
				String path = paths[i];
				String state = mVolumeStates.get(paths[i]);
				Slog.i(TAG, "will umount path =" + path + ";state = " + state);
    				if(state.equals(Environment.MEDIA_MOUNTED))
    				{
    				    rc |= doUnmountVolume(path, true,removeEncryption);
    				}
				}
		return rc;
	}
        ...
        synchronized (mVolumesLock) {
            ...
            if (state == null) {
                ...
                if (SystemProperties.get("vold.encrypt_progress").length() != 0) {
                   ...
                   //#ifndef VENDOR_EDIT
                    //LinJie.Xu@Prd.SysSrv.USB, 2012/06/21, Modify for 
                    /*
    ...
    public void unmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        //#ifndef VENDOR_EDIT
        //LinJie.Xu@Prd.SysSrv.USB, 2013/01/09, Remove for we don't support unmount swap
        /*
        if (FeatureOption.MTK_2SDCARD_SWAP && isExternalStorage(path) && 
                    path.equals(Environment.getExternalStorageDirectory().getPath())) {
            mUnmountSwap = true;
            updateDefaultPathForSwap(false, path);
        }
        */
    ...
    public void unmountVolumeNotSwap(String path, boolean force, boolean removeEncryption) {
        ...
        //#ifdef VENDOR_EDIT
        //LinJie.Xu@Prd.SysSrv.USB, 2013/01/31, Add for  wo should unmount /mnt/sdcard0/external_sd before /mnt/sdcard0/ 
            if (FeatureOption.OPPO_EMMC_DYNAMICMOUNT_SUPPORT && path.equals(OppoEnvironment.PARENT_STORAGE_DIRECTORY.getPath()))
            {
                String childState = getVolumeState(OppoEnvironment.SUB_STORAGE_DIRECTORY.getPath());
               if (Environment.MEDIA_MOUNTED.equals(childState))
               {
                    doUnmountVolume(OppoEnvironment.SUB_STORAGE_DIRECTORY.getPath(),force,removeEncryption);
               }
