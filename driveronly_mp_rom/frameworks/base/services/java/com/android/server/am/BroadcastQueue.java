...
public class BroadcastQueue {
    ...
    private final void deliverToRegisteredReceiverLocked(BroadcastRecord r,
        ...
        //OppoHook
        //ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/21
        if (OppoProcessManagerHelper.skipBroadcast(filter, r, ordered)) {
            if (DEBUG_BROADCAST) {
                Slog.v(TAG, "skip broadcast " + filter.packageName);
            }
            skip = true;
        }
        //#endif VENDOR_EDIT
...
//#ifdef VENDOR_EDIT
//Zhanhong.Hu@SystemSrv.AMS, 2012/09/13, Add for debug timeout before porcess ready,except the PRE_BOOT_COMPLETED.[OTA] 
                if (!mService.mProcessesReady && !Intent.ACTION_PRE_BOOT_COMPLETED.equals(r.intent.getAction())
                        && r.dispatchTime > 0) {
                    long now = SystemClock.uptimeMillis();
                                            
                        Slog.w(TAG, "huzhanhong add for: Hung broadcast discarded after timeout failure:"
                                + " now=" + now
                                + " dispatchTime=" + r.dispatchTime
                                + " startTime=" + r.receiverTime
                                + " intent=" + r.intent
...
//#endif /* VENDOR_EDIT */				
			...
			//OppoHook
        	//ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/21
            if (OppoProcessManagerHelper.checkBroadcastIsPackageCanStart(this, info, r)) {
                return;
            } else if (OppoProcessManagerHelper.checkAutoBootForbiddenStart(this, info, r)) {
                return;
            } else
            //#endif VENDOR_EDIT
...
//#ifndef VENDOR_EDIT
//Zhanhong.Hu@SystemSrv.AMS, 2012/09/13, Modify for keep the same logic if PRE_BOOT_COMPLETED timeout.[OTA]
        /*
...
//#else /* VENDOR_EDIT */
             if (!mService.mProcessesReady && Intent.ACTION_PRE_BOOT_COMPLETED.equals(r.intent.getAction())) {
                ...
                // PRE_BOOT_COMPLETED broadcasts can't timeout as they are intended
                // to do heavy lifting for system up.
                Slog.v(TAG, "mProcessesReady false, and this intent is PRE_BOOT_COMPLETED, return.");
                return;
            }
//#endif /* VENDOR_EDIT */
...
//Zhanhong.Hu@SystemSrv.AMS, 2012/09/13, Modify for show the ANR dialog after upgrade done.[OTA]
/*
        if (anrMessage != null) {
...
*/
//#else /* VENDOR_EDIT */
        if (mService.mProcessesReady && anrMessage != null) {
            //#ifdef VENDOR_EDIT
            //OppoHook
        	//ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/21
			if (OppoProcessManagerHelper.checkProcessWhileBroadcastTimeout(app)) {
				return;
			}
            //#endif VENDOR_EDIT
