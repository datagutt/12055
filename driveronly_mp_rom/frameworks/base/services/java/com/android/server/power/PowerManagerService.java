...
//#ifdef VENDOR_EDIT
//#@OppoHook
//#TongXi.Li@Plf.Framework, 2013/7/3, add for assert_lock                        
...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_FIELD,
                           ...
                           note="TongXi.Li@Plf.Framework for assert_lock")                      
    ...
    //#ifdef VENDOR_EDIT
	//#@OppoHook
	//#TongXi.Li@Plf.Framework.Assert, 2013/9/23, remove for button light
	...
	int getScreenBrightDefault(){
	    return mScreenBrightnessSettingDefault;
	}
	@android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_CLASS,
		                property=android.annotation.OppoHook.OppoRomType.ROM,
				        note="TongXi.Li@Plf.Framework, add for button light")
	static class OppoHelper {
		static LightsService.Light mButtonLight;
		static void setButtonLight( LightsService lightsService ){
		    mButtonLight= lightsService.getLight(LightsService.LIGHT_ID_BUTTONS);
		}
		static void updateButtonBrightness(PowerManagerService service, boolean condition ) {
			int tmp = service.getUserActivitySumm();
			boolean b1 = ((tmp & service.USER_ACTIVITY_SCREEN_DIM) != 0);
			boolean b2 =  (tmp == 0);
			boolean b = (service.getWakefulness() == service.WAKEFULNESS_AWAKE);

			if (condition){
			    if(b && !b1 && !b2){
			        mButtonLight.setBrightness(service.getScreenBrightDefault());
			    }
			}else{
			    mButtonLight.setBrightness(service.getScreenBrightDefault());
			}
    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        synchronized (mLock) {
            ...
            //#ifdef VENDOR_EDIT
            //#@OppoHook            
            //#TongXi.Li@Plf.Framework, 2013/7/10, add for button light  
            OppoHelper.setButtonLight(mLightsService);
            //#endif /* VENDOR_EDIT */
    ...
    private void acquireWakeLockInternal(IBinder lock, int flags, String tag, WorkSource ws,
        ...
        synchronized (mLock) {
            ...
            if (index >= 0) {
				...
				//#@OppoHook
				//#TongXi.Li@Plf.Framework, 2013/7/3, add for assert_lock
                wakeLock.mAssertTime = SystemClock.uptimeMillis();
				//#endif /* VENDOR_EDIT */
    ...
    //#ifdef VENDOR_EDIT
    //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/26, Add for 
    private boolean bIgnoreProxBecauseScreenTurnedOffDutToUserRequest = false;
    private int mGoToSleepReason = 0;
    //#endif /* VENDOR_EDIT */
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason) {
        ...
        //#ifdef VENDOR_EDIT
        //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/26, Add for 
        bIgnoreProxBecauseScreenTurnedOffDutToUserRequest = false;
        //#endif /* VENDOR_EDIT */
        ...
        switch (reason) {
            ...
            case PowerManager.GO_TO_SLEEP_REASON_TIMEOUT:
                ...
                break;
            //#ifdef VENDOR_EDIT
            //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/04/02, Add for 
            case PowerManager.GO_TO_SLEEP_REASON_PROX_SENSOR:
            ...
            default:
                ...
                if (shouldUseProximitySensorLocked()) {
                    bIgnoreProxBecauseScreenTurnedOffDutToUserRequest = true;
                }   
                //#endif /* VENDOR_EDIT */
        ...
        //#ifdef VENDOR_EDIT
        //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/04/13, Add for 
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_CODE,
                             property=android.annotation.OppoHook.OppoRomType.ROM,
                             note="TongXi.Li@Plf.Framework for assert_lock and button light")
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_WAKEFULNESS)) != 0) {
            ...
            for (int i = 0; i < numWakeLocks; i++) {
                ...
                switch (wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK) {
                    ...
                    case PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK:
                        //#ifndef VENDOR_EDIT
                        //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/12, Modify for 
                        /*
                        if (mWakefulness != WAKEFULNESS_ASLEEP) {
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS | DIRTY_SETTINGS)) != 0) {
            ...
            //#ifdef VENDOR_EDIT
            //#@OppoHook            
            //#TongXi.Li@Plf.Framework.Assert, 2013/7/10, add for button light  
            OppoHelper.updateButtonBrightness(this, true);
            //#endif /* VENDOR_EDIT */ 
            if (DEBUG_SPEW) {
    ...
    private boolean updateWakefulnessLocked(int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_BOOT_COMPLETED
                ...
                //#ifndef VENDOR_EDIT
                //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/04/03, Remove for 
                /*
    ...
    //#ifdef VENDOR_EDIT
    //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/01/26, Add for 
    /* this function for get lcd brightness for camera */
    public int getLcdBrightness() {
        synchronized (mLock) {
        
            int currentLcdBrightness = mScreenBrightnessSettingDefault;
        
            if (mDisplayPowerRequest.useAutoBrightness)
            {
                currentLcdBrightness = (int) (mScreenBrightnessSettingMaximum 
                                * mDisplayPowerRequest.screenAutoBrightnessAdjustment);
            } else {
            
                currentLcdBrightness = mDisplayPowerRequest.screenBrightness;
            }
            
            return currentLcdBrightness;
        }
    }

    public void syncProximityPositiveState(boolean positive) {
        //for sync Proximity Positive State with some other already register listener
        mDisplayPowerControllerCallbacks.onProximityPositive();
        boolean PROXIMITY_POSITIVE = positive ? true : false;
        mDisplayPowerController.setSyncPendingProximity(PROXIMITY_POSITIVE);
    }
    //#endif /* VENDOR_EDIT */
    ...
    private void updateDisplayPowerStateLocked(int dirty) {
        //#ifndef VENDOR_EDIT
        //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/14, Modify for 
        /*
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS
        */
        //#else /* VENDOR_EDIT */
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS | DIRTY_PROXIMITY_POSITIVE
        //#endif /* VENDOR_EDIT */
    ...
    //#ifdef VENDOR_EDIT
    //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/26, Add for 
    public boolean bIsScreenOffBecauseOfProximityPositive() {
        Slog.d(TAG, "bIsScreenOffBecauseOfProximityPositive: shouldUseProximitySensorLocked :"
            + shouldUseProximitySensorLocked() + ", mProximityPositive = " + mProximityPositive);
        
        return (mGoToSleepReason == PowerManager.GO_TO_SLEEP_REASON_PROX_SENSOR) 
                && (mWakefulness == WAKEFULNESS_ASLEEP)
    ...
    private final DisplayPowerController.Callbacks mDisplayPowerControllerCallbacks =
        ...
        public void onStateChanged() {
            synchronized (mLock) {
                //#ifdef VENDOR_EDIT
                //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/04/22, Add for 
                Slog.d(TAG, "onStateChanged");
                //#endif /* VENDOR_EDIT */
        ...
        public void onProximityPositive() {
            synchronized (mLock) {
                ...
                //#ifndef VENDOR_EDIT
                //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/29, Modify for 
                /*
                updatePowerStateLocked();
                */
                //#else /* VENDOR_EDIT */
                Slog.d(TAG, "onProximityPositive: goToSleepInternal");
                goToSleepInternal(SystemClock.uptimeMillis(), PowerManager.GO_TO_SLEEP_REASON_PROX_SENSOR);
                //#endif /* VENDOR_EDIT */
            }
        }
        ...
        public void onProximityNegative() {
            synchronized (mLock) {
                ...
                //Jianrong.Zheng@Prd.BasicSrv.BatteryManager, 2013/03/26, Add for 
                if (mWakefulness == WAKEFULNESS_ASLEEP 
                    && bIgnoreProxBecauseScreenTurnedOffDutToUserRequest) {

                    if (DEBUG_SPEW) {
                        Slog.d(TAG, "onProximityNegative: dismiss proximity wake lock because of uer request");
                    }
                ...
                */
                //#else /* VENDOR_EDIT */
                if (mWakefulness != WAKEFULNESS_AWAKE) {      
                    Slog.d(TAG, "onProximityNegative: wakeUpInternal");
                    wakeUpInternal(SystemClock.uptimeMillis());
                } else {       
                    Slog.d(TAG, "onProximityNegative: userActivityInternal");
                    userActivityInternal(SystemClock.uptimeMillis(),
                            PowerManager.USER_ACTIVITY_EVENT_OTHER, 0, Process.SYSTEM_UID);
                }
                //#endif /* VENDOR_EDIT */
            }
...
//#ifdef VENDOR_EDIT
//JiangLin.Ma@Prd.SysApp.Settings, 2013/03/30, Add for 
    public int getOppoBatteryLevel() {
...
//#endif /* VENDOR_EDIT */
    ...
    //#@OppoHook
    //TongXi.Li@Plf.Framework, 2013/7/3, Add for assert_lock                         
    private static final String     ATAG     = "AssertLock";
    private static final boolean    ADBG     = (SystemProperties.getBoolean("persist.sys.assert.enable", false)
                        || SystemProperties.getBoolean("persist.sys.assert.panic", false));
    private static final long       A_STEP   = (5L * 60000L)  ;  
    private static final long       A_ASSERT = (10L * 60000L) ;    
    final private String[] mAssertUncheck    = new String[] {
            "StayOnWhilePluggedIn Partial"
        ,   "StayOnWhilePluggedIn Screen Dim"
        ,   "android.media.MediaPlayer"
        ,   "MediaScannerService"               // bug 219347 10089-9869
        ,   "oppo.multimedia.soundrecorder.RecorderService"
        ,   "BtOppObexServer"                   // BluetoothOppObexClientSession
        ,   "BtOpp ObexClient"                  // BluetoothOppObexClientSession
        ,   "StartingObexPbapTransaction"       // BluetoothPbapService
        ,   "DownloadManager"                   // Download Provider
        ,   "OppoCsvExportRunnable"             // Phone.book
        ,   "GpsLocationProvider"	            // bug 209454 10089-6836
        ,   "SyncManagerSyncWakeLock"	        // com.oppo.android.sync
        ,   "NetworkLocation Check Location"    // google application
        ,   "CopyFileWakeLock"                  // com.oppo.filemanager
        ,   "daemon.l.a"                        // 9 1  helper
        ,   ""
	};	
    private Runnable mAssertCheck = new Runnable() {
        public void run() {
            synchronized (mLock) {                
                if ((SystemClock.elapsedRealtime() - mLastSleepTime) < A_ASSERT){                    
                    mHandler.postDelayed(this, A_STEP);
                    return ;
                }                
                assertCheck();
                mHandler.postDelayed(this, A_STEP);
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
                             property=android.annotation.OppoHook.OppoRomType.ROM,
                             note="TongXi.Li@Plf.Framework for assert_lock")
    private void assertOnScreenOn(){
        synchronized (mLock) {            
            mHandler.removeCallbacks(mAssertCheck);
        }
    }
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
                             property=android.annotation.OppoHook.OppoRomType.ROM,
                             note="TongXi.Li@Plf.Framework for assert_lock")
    private void assertOnScreenOff(){
        synchronized (mLock) {                       
            if (mWakeLocks.size() > 0){                
                mHandler.postDelayed(mAssertCheck,A_ASSERT);                
            }
        }
    }
    ...
    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
                             property=android.annotation.OppoHook.OppoRomType.ROM,
                             note="TongXi.Li@Plf.Framework for assert_lock")
	private RunningAppProcessInfo assertUidInfo(int pid, int uid) {	    
		java.util.List<RunningAppProcessInfo> apps = mActivityService.getRunningAppProcesses();
    ...
    private void assertCheck()
    {
        ...
        for (int i=0; i<N; i++) {
            ...
            for (int j=0; !ingore && (j < mAssertUncheck.length); j++){
                ingore |= wl.mTag.equals(mAssertUncheck[j]);
                ingore |= ((0 != (wl.mFlags & PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK)) ? true : false);
            }
            if (ingore){
                Slog.w(ATAG, "Uncheck " + wl.mTag
                        + " hold walk " + (now - wl.mAssertTime) + " ms.");
                continue ;
            }            
            long hold = (now - wl.mAssertTime);
            if (hold > A_STEP) {
                StringBuilder sb = new StringBuilder();
                String fileName = "";
                sb.append("\n");                
                RunningAppProcessInfo info = assertUidInfo(wl.mOwnerPid, wl.mOwnerUid);
                if (info != null){
                    sb.append("\"");
                    sb.append(info.processName);
                    fileName = info.processName;
                    sb.append("\" process ");
                } else {
                    sb.append(wl.mOwnerUid);
                    sb.append(" uid ");
                    fileName = "uid_"+wl.mOwnerUid;
                }
                sb.append("\"");
                sb.append(wl.mTag);
                sb.append("\" hold walk ");
                sb.append(hold); sb.append(" ms!\n");                
                sb.append(" when screen off.");                
                Slog.e(ATAG, sb.toString());                
				//ASSERT.ASSERT(sb.toString(), false);
				ASSERT.fail(sb.toString(), fileName);
            }
        }
    }
}
