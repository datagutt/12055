...
public class UsbDeviceManager {
    ...
    private final UEventObserver mUEventObserver = new UEventObserver() {
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            ...
            //#ifdef VENDOR_EDIT
            //TongXi.Li@Plf.Framework, 2012/10/24, Modify for resolve bug:98286 oppo_usb
            /*
			if (state != null) {
			*/							
			//#else /* VENDOR_EDIT */
            if (state != null && mHandler != null) {
            //#endif /* VENDOR_EDIT */            
    ...
    public UsbDeviceManager(Context context) {
        ...
        //#else /* VENDOR_EDIT */
        mHandler = new OppoUsbHandler(thread.getLooper());
        //#endif /* VENDOR_EDIT */    
    ...
    class UsbHandler extends Handler {
		...
		*/							
        ...
        UsbAccessory mCurrentAccessory;
        int mUsbNotificationId;
        boolean mAdbNotificationShown;
        int mCurrentUser = UserHandle.USER_NULL;
        int mPlugType;
		//#endif /* VENDOR_EDIT */ 
        ...
        @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_ACCESS,
									property=android.annotation.OppoHook.OppoRomType.ROM,
            						note="private:TongXi.Li@Plf.Framework modify for oppo_usb") 
        boolean waitForState(String state) {
        ...
        @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_ACCESS,
									property=android.annotation.OppoHook.OppoRomType.ROM,
            						note="private:TongXi.Li@Plf.Framework modify for oppo_usb") 
        boolean setUsbConfig(String config) {
        ...
        @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_ACCESS,
									property=android.annotation.OppoHook.OppoRomType.ROM,
            						note="private:TongXi.Li@Plf.Framework modify for oppo_usb") 
        void setAdbEnabled(boolean enable) {
        ...
        @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_ACCESS,
									property=android.annotation.OppoHook.OppoRomType.ROM,
            						note="private:TongXi.Li@Plf.Framework modify for oppo_usb") 
        void setEnabledFunctions(String functions, boolean makeDefault) {
            if (DEBUG) {
        ...
        @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.CHANGE_ACCESS,
									property=android.annotation.OppoHook.OppoRomType.ROM,
            						note="private:TongXi.Li@Plf.Framework modify for oppo_usb") 
        void updateAudioSourceFunction() {
    ...
    private final class OppoUsbHandler extends UsbHandler{		
    	private static final String TAG1 = "QcomltxUsbDeviceManager";
        public OppoUsbHandler(Looper looper){
            super(looper); 
            mAdbEnabled = mAdbEnabled ||
                              (Settings.Global.getInt(mContentResolver,Settings.Global.ADB_ENABLED, 0) > 0);
        }
        @Override
        void setAdbEnabled(boolean enable) {           
            Slog.d(TAG1, ">>>enable = " + enable + ", mAdbEnabled = " + mAdbEnabled + ", mCurrentFunctions = " + mCurrentFunctions);
            Slog.w(TAG1, ">>>setAdbEnabled " + enable);
            if (enable != mAdbEnabled) {
                mAdbEnabled = enable;
                setEnabledFunctions(mDefaultFunctions, true);
            }
			if (mDebuggingManager != null) {
				...
				@Override
        void setEnabledFunctions(String functions, boolean makeDefault) {
            Slog.w(TAG1, ">>>setEnabledFunctions: functions = " + functions +
                "   mDefaultFun: " + mDefaultFunctions +"   mCurFun: " + mCurrentFunctions+
                "   mkDefault "+makeDefault);
            if (functions != null && makeDefault && !needsOemUsbOverride()) {
                ...
                if (!mDefaultFunctions.equals(functions)) {                    
                    if (!setUsbConfig("none")) {
                        Slog.e(TAG1, "Failed to disable USB");                        
                        setUsbConfig(mCurrentFunctions);
                        return;
                    }                   
                    SystemProperties.set("persist.sys.usb.config", functions);                    
                    if (waitForState(functions)) {
                        mCurrentFunctions = functions;
                        mDefaultFunctions = functions;
                        Slog.w(TAG1, ">>>setenfun success in if :" + functions);
                        Slog.w(TAG1, ">>>persist.sys.usb.config : " + SystemProperties.get("persist.sys.usb.config", "none"));
                    } else {
                        Slog.e(TAG1, ">>>Failed to switch persistent USB config to " + functions);
                        SystemProperties.set("persist.sys.usb.config", mDefaultFunctions);
                    }
                }
            } else {
                ...
                if (!mCurrentFunctions.equals(functions)) {                    
                    ...
                    if (setUsbConfig(functions)) {
                        mCurrentFunctions = functions;
                        Slog.w(TAG1, ">>>setenfun success in else :" + functions);
                    } else {
                        Slog.e(TAG1, "Failed to switch USB config to " + functions);
                        setUsbConfig(mCurrentFunctions);
                    }
                }
            }
