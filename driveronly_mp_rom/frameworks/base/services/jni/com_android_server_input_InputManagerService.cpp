...
static jintArray nativeGetTouchedWindowPids(JNIEnv* env, jclass clazz, jint ptr) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    Vector<int> pids;
    im->getInputManager()->getDispatcher()->getTouchedWindowsPids(pids);
	
    jintArray pidsObj = env->NewIntArray(pids.size());
    if (!pidsObj) {
        return NULL;
    }

    env->SetIntArrayRegion(pidsObj, 0, pids.size(), pids.array());
    return pidsObj;
}
//#endif /* VENDOR_EDIT */
...
static JNINativeMethod gInputManagerMethods[] = {
	...
	//property=android.annotation.OppoHook.OppoRomType.OPPO
	//Chenps ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/21
    { "nativeGetTouchedWindowPids", "(I)[I",
            (void*) nativeGetTouchedWindowPids },
    //#endif /* VENDOR_EDIT */
