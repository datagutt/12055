...
#else /* VENDOR_EDIT */
#ifdef ENG_BUILD
const nsecs_t DEFAULT_INPUT_DISPATCHING_TIMEOUT = 2500 * 1000000LL; // 8 sec
#else
const nsecs_t DEFAULT_INPUT_DISPATCHING_TIMEOUT = 8000 * 1000000LL; // 8 sec 
#endif
#endif /* VENDOR_EDIT */
...
int IsProcessInSuspendState(int pid)
{
...
int32_t InputDispatcher::findTouchedWindowTargetsLocked(nsecs_t currentTime,
    ...
    for (size_t i = 0; i < mTempTouchState.windows.size(); i++) {
        ...
        if (touchedWindow.targetFlags & InputTarget::FLAG_FOREGROUND) {
            ...
            if (touchedWindow.windowHandle->getInfo()->paused) {
                /// M: Enhance keydispatching predump 
        #ifdef VENDOR_EDIT
        //Jiangtao.Guo@Prd.SysSrv.InputManager, 2013/02/22, Add for longPressing(start activity) cause ANR 
                if (touchedWindow.windowHandle != mFocusedWindowHandle) {
                    //ALOGW("mFocusedWindowHandle = %s  touchedWindow.windowHandle = %s",
                    //        mFocusedWindowHandle->getName().string(),
                    //        touchedWindow.windowHandle->getName().string());
                    ALOGW("mFocusedWindowHandle != touchedWindow.windowHandle");
			        //mTempTouchState.removeWindow(touchedWindow.windowHandle);
			        //i--;
			        //continue;
			        resetANRTimeoutsLocked();
			        continue;
    ...
    for (size_t i = 0; i < mTempTouchState.windows.size(); i++) {
		...
		//property=android.annotation.OppoHook.OppoRomType.OPPO
    	//Chenps ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/21
        int pid = touchedWindow.windowHandle->getInfo()->ownerPid;
		if (IsProcessInSuspendState(pid)) {
		    kill(pid, 18);
		}
        //#endif /* VENDOR_EDIT */
    }
...
void InputDispatcher::dumpDispatchStateLocked(String8& dump) {
    ...
    nsecs_t currentTime = now();
    #ifdef VENDOR_EDIT
    //Jiangtao.Guo@Prd.SysSrv.InputManager, 2013/03/19, Add for ANR info
        int num = 0;
    #endif /* VENDOR_EDIT */
    if (!mInboundQueue.isEmpty()) {
        ...
        for (EventEntry* entry = mInboundQueue.head; entry; entry = entry->next) {
            ...
            #ifdef VENDOR_EDIT
            //Jiangtao.Guo@Prd.SysSrv.InputManager, 2013/03/19, Add for 
            num = num +1;
            if(num > 5){
                dump.append(INDENT "more ... ... \n");
                break;
            }
            #endif /* VENDOR_EDIT */
        }
...
#ifdef VENDOR_EDIT
//Jiangtao.Guo@Prd.SysSrv.InputManager, 2013/03/18, Add for ANR info
    char* text = mLastANRState.lockBuffer(mLastANRState.size());
    char* start = text;
    while (*start != '\0') {
        char* end = strchr(start, '\n');
        if (*end == '\n') {
...
//#ifdef VENDOR_EDIT
//Zhanhong.Hu@Prd.SystemSrv.BPM, 2013/01/21
void InputDispatcher::getTouchedWindowsPids(Vector<int32_t>& outPids) {
    AutoMutex _l(mLock);
    ...
    if (!mTouchState.windows.isEmpty()) {
        for (size_t i = 0; i < mTouchState.windows.size(); i++) {
            const TouchedWindow& touchedWindow = mTouchState.windows[i];
            outPids.add(touchedWindow.windowHandle->getInfo()->ownerPid);
        }
    }
}
//#endif /* VENDOR_EDIT */
