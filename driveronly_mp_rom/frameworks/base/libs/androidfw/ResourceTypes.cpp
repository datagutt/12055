...
#include <androidfw/ThemeResMap.h>
...
#ifdef VENDOR_EDIT
//ZhongHaiping@Prd.MidWare.Theme,2012/12/28,for theme, upgrade android4.1
		//Pihe.Wu@Prd.MidWare.Theme, 2012/08/04, Add for: transform new colorsmap solustion
			char filePath[120];
			property_get("persist.sys.skin", filePath, DEFAULT_THEME_PATH);
			if (strcmp(filePath, DEFAULT_THEME_PATH) != 0)
			{		
				ResTable::resource_name name;
				getResourceName(resID, &name);
				OPPOThemeManager* oppoThemeManager = OPPOThemeManager::getInstance();
				char* lastThemePath = oppoThemeManager->getLastThemePath();
				if (strcmp(filePath, lastThemePath) != 0) 
				{
					oppoThemeManager->parseColorsFileIfNeeded();
					oppoThemeManager->setLastThemePath(filePath);
				}
		
				if (outValue->dataType >= bestValue->TYPE_FIRST_COLOR_INT && 
						 outValue->dataType <= bestValue->TYPE_LAST_COLOR_INT)
				{
					int value = oppoThemeManager->findColorByColorName(String8(String16(name.name,name.nameLen)).string());
					if (value != -1) 
					{				
						outValue->data = value;
					}
