...
#else  /* VENDOR_EDIT */
static const char* kDefaultThemePath = "/system/framework/framework-res.apk";
//Liguan@Prd.MidWare.Theme,2012/10/24,Remove for using default theme package launcher icons while in default theme
static const char* kDefaultLauncherThemePath = "/system/framework/oppo.theme-res-default.apk";
static char kThemeApkPath[100];
//Liguan@Prd.MidWare.Theme,2013/01/15,Add for Launcher icons of nearMe applications won't change as the theme changed
static const char* nearmeMap[6][2] = {
						{"/data/app/com.oppo.market", "/system/app/NearMeMarket.apk"},
						{"/data/app/com.oppo.musiconline", "/system/app/NearMeMusic.apk"},
						{"/data/app/com.nearme.note", "/system/app/NearMeNote.apk"},						
						{"/data/app/com.oppo.reader", "/system/app/NearMeReader.apk"},
						{"/data/app/com.nearme.sync", "/system/app/NearMeSync.apk"},	
						{"/data/app/com.nearme.gamecenter", "/system/app/GameCenter.apk"}	};
static const char* kDefaultThemeVersion = "4";
static char KThemeVersion[10];

static const char* kISmsPath = "/data/app/com.hissage-";
static const char* kHdpi="res/drawable-hdpi";
static const char kDrawable='/'; 
#endif /* VENDOR_EDIT */
namespace {
...
*/
 ...
 #else
    ALOGV("THEME_NO_BUILD is not set in device and parse here .mThemeManager =  %d.", mThemeManager);
    mThemeManager = OPPOThemeManager::getInstance();
	mThemeManager->parseThemeMapIfNeeded();
 #endif
#endif /* VENDOR_EDIT */
...
//		Asset* pAsset = getAssetFromTheme(which, fileName, *mThemePath, mode);
	...
	#ifdef VENDOR_EDIT
		Asset* pAsset = NULL;
        //Liguan@Prd.MidWare.Theme,2012/10/24,Modify for using default theme package launcher icons while in default theme
	    pAsset = getAssetFromTheme(which, fileName, *mThemePath, mode);
...
#ifdef VENDOR_EDIT
//yaoshuang.zhang@Prd.SysSrv.PMS,2012/8/3,porting
//zyx:add for apk encryption
void AssetManager::getAssetPaths(Vector<String8>* assetPaths) const
{
	for (size_t i=0; i<mAssetPaths.size(); i++)
	{
		assetPaths->add(mAssetPaths[i].path);
	}
}
// zyx add end
#endif // VENDOR_EDIT
...
#ifdef VENDOR_EDIT
//ZhongHaiping@Prd.MidWare.Theme,2012/12/28,for theme, upgrade android4.1
	//Pihe.Wu@CommonUI.Res, 2012/06/11, Add for bug411980,
		//when theme changed the apk in this dir was remove,so it will return false.
		//but it will let the getTopLevelResources function make a new resources instance
		//in this case some apks which use it laungage changed failed,so just filter it.
		if(mPath.find("/data/oppo/OPPO_THEME_CHANGE/framework/",0) != -1){
			//LOGD("find the theme %s",mPath.string());
			return true;
		}
#endif /* VENDOR_EDIT */
...
Asset* AssetManager::getAssetFromTheme(int which, const char* fileName,
	...
	Asset* pAsset = NULL;
	OPPOThemeManager *mThemeManager = OPPOThemeManager::getInstance();
	property_get("persist.sys.themeVersion", KThemeVersion, kDefaultThemeVersion);
	mThemeManager->setThemeVersion(atoi(KThemeVersion));
    #ifdef VENDOR_EDIT
    //Liguan@Prd.MidWare.Theme,2013/01/15,Add for Launcher icons of nearMe applications won't change as the theme changed
    bool isSetup = false;
    const char *pathTemp  = mAssetPaths.itemAt(which).path.string();

    for(int i = 0; i < 6; i++)
        {
            if(strstr(pathTemp, nearmeMap[i][0]))
            {
                isSetup = true;
...
//    }
	...
	if ((mThemeManager != NULL && mThemeManager->isFileInMap(mAssetPaths.itemAt(which).path.string(),
			fileName))  ) 
	#else
	if (mThemeManager != NULL && mThemeManager->isFileInMap(mAssetPaths.itemAt(which).path.string(),
			fileName) 
...
/*
		if (which == 0x00) { // frameworks resource use default filename to access
			...
			appendApkPath((char*) mAssetPaths.itemAt(which).path.string(),
...
*/
#else  /* VENDOR_EDIT */
    //Liguan@Prd.MidWare.Theme,2012/10/24,add for using default theme package launcher icons while in default theme
    char themePathTemp[256];
		char propVal[10];
		property_get("persist.sys.editorType", propVal, "0");
		int editorType = atoi(propVal);

		char* apkFileName = (char*) malloc(strlen(fileName) + 60);
		memset(apkFileName, 0x00, strlen(fileName) + 60);
		strncpy(apkFileName, fileName, strlen(fileName));
	    ALOGV("apkFileName = %s\n", apkFileName);
		replaceStr(apkFileName, kDrawable,kHdpi);
		ALOGV("replaceStr apkFileName = %s\n", apkFileName);
#ifdef VENDOR_EDIT
			//Liguan@Prd.MidWare.Theme,2012/10/24,add for using default theme package launcher icons while in default theme
        if (strcmp(kThemeApkPath, kDefaultThemePath) == 0) {
                strcpy(themePathTemp, themePath.path.string());
		        themePath.path.setTo(kDefaultLauncherThemePath);
			    pAsset = openNonAssetInPathLocked(apkFileName, mode, themePath);
			    if (pAsset == NULL) {
			        themePath.path.setTo(themePathTemp);
			    }
	    }
#endif
...
#ifdef VENDOR_EDIT
//ZhongHaiping@Prd.MidWare.Theme,2012/12/28,for theme, upgrade android4.1
int AssetManager::replaceStr(char *sSrc, const char sMatchStr, const char *sReplaceStr)
{
    char caNewString[256];
    char *pFind  = strrchr(sSrc, sMatchStr);
    if((pFind == NULL) || (!pFind))
           return -1;	
	memset(caNewString, 0, sizeof(caNewString));
	strcat(caNewString, sReplaceStr);
	strcat(caNewString, pFind);
