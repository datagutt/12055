...
#ifdef VENDOR_EDIT
//wenhua.Leng@oppo.RD.GrapSrv.hwui add at 2013/09/16 for texture cache size		
...
//wenhua.Leng@oppo.RD.GrapSrv.hwui add at 2013/09/16 for texture cache size		
		...
		int totalMem = s_info.totalram/1024;
		if(error == 0){
			if(totalMem > 1024){
				setMaxSize(MB(MORE_THAN_1GB_TEXTURE_CACHE_SIZE));
				INIT_LOGD("Mem more than 1024MB,Setting texture cache size to %sMB", MORE_THAN_1GB_TEXTURE_CACHE_SIZE);
			}else if(totalMem > 512){
				setMaxSize(MB(MORE_THAN_512MB_TEXTURE_CACHE_SIZE));
				INIT_LOGD("Mem more than 512MB,Setting texture cache size to %sMB", MORE_THAN_512MB_TEXTURE_CACHE_SIZE);
			}else{
				INIT_LOGD("  Using default texture cache size of %.2fMB", DEFAULT_TEXTURE_CACHE_SIZE);
			}
		}else{
			INIT_LOGD("system call fail, Using default texture cache size of %.2fMB", DEFAULT_TEXTURE_CACHE_SIZE);
		}
#endif /*VENDOR_EDIT*/		
    }
