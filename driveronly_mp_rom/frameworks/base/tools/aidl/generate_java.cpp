...
gather_comments(extra_text_type* extra)
{
    ...
    while (extra) {
        if (extra->which == SHORT_COMMENT) {
           ...
           //#ifdef VENDOR_EDIT
           //#@OppoHook
           //#Jianjun.Dan@Plf.Framework.SDK add if 
           if (0 == strncmp("@+", extra->data, 2)) {
               string anno = extra->data;
               int pos =  anno.find_first_of("@",2);
               int colon = anno.find_first_of("\"",2) + 1;
               int end =  anno.find_first_of("\"",colon) + 1;
               if (anno[end] != ')') {
                   anno[end] = ')';
               }
               end++;
               ...
               if (size > end) {
                   s += "/*";
                   s += anno.substr(end, size - end);
                   s += "*/ \n";
               }
               s += anno.substr(pos, end - pos);
           } else {
               s += "/*";
               s += extra->data;
               s += "*/";
