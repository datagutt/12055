...
status_t ResourceTable::addIncludedResources(Bundle* bundle, const sp<AaptAssets>& assets)
{
    ...
    //add by danjianjun 
    if (mBundle -> getOppoPackage() ) {
        if (mNextPackageId == mBundle -> getOppoPackageId()) {
            mNextPackageId++;
        }
    }
    //add end 2012-11-27
...
status_t ResourceTable::assignResourceIds()
{
    ...
    for (pi=0; pi<N; pi++) {
        ...
        for (ti=0; ti<N; ti++) {
			...
			//added by danjianjun 2012-05-28
			
			if (mBundle->getOppoPublic()) {
				t->setOppoPublic(true,mBundle->getOppoPublicBaseId());
			} else {
				t->setOppoPublic(false,mBundle->getOppoPublicBaseId());
			}
			//added by danjianjun 2012-05-28
        ...
        for (ti=0; ti<N; ti++) {
            ...
            for (size_t ci=0; ci<N; ci++) {
                ...
                if (Res_GETPACKAGE(rid) == (size_t)(p->getAssignedId()-1)) {
					//added by danjianjun 2012-05-28
					if (mBundle->getOppoPublic() && !c->getPublic() ) {
						if (c->getOppoTemp()){
							continue;
						}
					}
					//added end by danjianjun 2012-05-28
        ...
        for (size_t ti=0; ti<N; ti++) {
            ...
            for (size_t i = 0; i < N; ++i) {
                if (!validResources[i]) {
                    ...
                    //added by danjianjun 2013-06-15
                    if (c->getOppoTemp() && mBundle->getOppoPublic()) {
                        continue;
                    }
                    //added end by danjianjun 2013-06-15
...
//added by danjianjun  2012-05-28
void ResourceTable::Type::addTempOrderedConfigs() {
	const size_t NP = mPublic.size();
	const size_t N = mOrderedConfigs.size();
	sp<ConfigList> e = mOrderedConfigs.itemAt(0);
	const String16& name = e->getName();
	char suffix[20] = {0};
	size_t i =0;
	size_t maxId = mOppoPublicBaseId;

	for (i=0; i<NP; i++) {
		const String16& name = mPublic.keyAt(i);
       	const Public& p = mPublic.valueAt(i);
       	size_t id = Res_GETENTRY(p.ident);
		if (id > N && id < mOppoPublicBaseId) {
			fprintf(stderr, "ERROR: oppo public id %s 0x%08x greater than 0x%08x\n", String8(name).string(), p.ident, p.ident - id + mOppoPublicBaseId);
		}
		if (id > maxId) {
			maxId = id;
		}
	...
	for (i=N; i<= maxId; i++) {
		sprintf(suffix, "%s%d", "_assign_tmp_id_", i);
		String16 oppoName = String16(name);
		oppoName.append(String16(suffix));
		ConfigList* oppoTemp = new ConfigList(oppoName, e->getPos());
		oppoTemp->setOppoTemp(true);
       	mOrderedConfigs.add(oppoTemp);
    }
}
//added end by danjianjun  2012-05-28
...
status_t ResourceTable::Type::applyPublicEntryOrder()
{
    ...
    j = 0;
	//added by danjianjun 2012-05-30
	size_t oppo = mOppoPublicBaseId - 1;  
	size_t base = mOppoPublicBaseId;
	//added end by danjianjun 2012-05-30
    for (i=0; i<N; i++) {
        sp<ConfigList> e = origOrder.itemAt(i);
        // There will always be enough room for the remaining entries.
		//added by danjianjun 2012-05-30
		if (mOppoPublic) {
			if (0 == strncmp("oppo_", String8(e->getName()).string(), 5)) {
				bool isReplace =false;
				while (base < mOrderedConfigs.size() && !isReplace) {
            		if (mOrderedConfigs.itemAt(base) == NULL) {
						mOrderedConfigs.replaceAt(e, base);
						isReplace = true;
					}
					base++;
				} 
				if (!isReplace) {
					...
					String16 tmpName = String16(e->getName());
					tmpName.append(String16("_tmp_internal"));
					ConfigList* oppoTemp = new ConfigList(tmpName, e->getPos());
					oppoTemp->setOppoTemp(true);
					while (mOrderedConfigs.itemAt(oppo) != NULL) {
            			oppo--;
        			}
					mOrderedConfigs.replaceAt(oppoTemp, oppo);
					oppo--;
				}
				continue;
			}
...
sp<ResourceTable::Package> ResourceTable::getPackage(const String16& package)
{
    ...
    if (p == NULL) {
        if (mIsAppPackage) {
		    ...
		    // add by danjianjun 
            } else if (mBundle -> getOppoPackage()) {
			   p = new Package(package, mBundle -> getOppoPackageId());
			   //add end 2012-11-27
			} else {
        ...
        //modify by danjianjun  add if-else
        if (mBundle -> getOppoPackage() ) {
	     if (mNextPackageId == mBundle -> getOppoPackageId()) {
                mNextPackageId++;
	     }
