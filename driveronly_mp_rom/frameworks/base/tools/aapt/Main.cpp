    ...
    while (argc && argv[0][0] == '-') {
        ...
        while (*cp != '\0') {
            ...
            case '-':
                ...
                } else if (strcmp(cp, "-oppo-public-id") == 0) { //added by danjianjun  2012-05-25
					argc--;
                    argv++;
					if (!argc) {
                        fprintf(stderr, "ERROR: No argument supplied for '-oppo-public-id' option\n");
                        wantUsage = true;
                        goto bail;
                    }
					unsigned int result = atoi(argv[0]); 
					printf("set oppo public base id:%s:0x%04x\n", argv[0], result);
                    bundle.setOppoPublicBaseId(result); 
					bundle.setOppoPublic(true);
                    //added end by danjianjun 2012-05-25
                }  else if (strcmp(cp, "-oppo-package") == 0) { //added by danjianjun  2012-11-27
					argc--;
                    argv++;
					if (!argc) {
                        fprintf(stderr, "ERROR: No argument supplied for '-oppo-package' option\n");
                        wantUsage = true;
                        goto bail;
                    }
					unsigned int result = atoi(argv[0]); 
					printf("set oppo package id : %s : 0x%02x\n", argv[0], result);
					if (result > 126 || result < 2) {
                        fprintf(stderr, "ERROR: oppo package id :%d must be less than 126 and greater than 1\n", result);
						wantUsage = false;
						goto bail;
					}
                    bundle.setOppoPackageId(result); 
					bundle.setOppoPackage(true);
                    //added end by danjianjun 2012-11-27
                } else {
