...
public:
    ...
    public:
        ConfigList(const String16& name, const SourcePos& pos)
            : mName(name), mPos(pos), mPublic(false), mEntryIndex(-1), mOppoTemp(false) { } //added mOppoTmp by danjianjun 2012-05-28
        virtual ~ConfigList() { }
		...
		void setOppoTemp(const bool temp) { mOppoTemp = temp; }
		bool getOppoTemp() const { return mOppoTemp; }
    ...
    private:
		...
		bool mOppoTemp; //added by danjianjun 2012-05-28
    };
    ...
    public:
        Type(const String16& name, const SourcePos& pos)
                : mName(name), mFirstPublicSourcePos(NULL), mPublicIndex(-1), mIndex(-1), mPos(pos),
				 mOppoPublic(false), mOppoPublicBaseId(2048) //added by danjianjun 2012-05-28
        { }
        ...
        const SourcePos& getPos() const { return mPos; }
		//added by danjianjun 2012-05-28
		void setOppoPublic(const bool isPublic, const size_t baseId) {mOppoPublic = isPublic; mOppoPublicBaseId = baseId; }
		void addTempOrderedConfigs();
		//addedn end by danjianjun 2012-05-28
    private:
        String16 mName;
