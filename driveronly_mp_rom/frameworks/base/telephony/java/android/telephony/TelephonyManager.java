...
public class TelephonyManager {
   ...
   public static final int RADIO_MISC = 2;
    ...
    //#ifndef VENDOR_EDIT
    //OPPOROM@Plf.CommSrv.Telephony, 2012/01/07, modify from private to  protected
    ...
    public void enableLocationUpdates() {
		...
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
        if(mtkGeminiSupport){
            enableLocationUpdatesGemini(getDefaultSim());
        } else
        //#endif /* VENDOR_EDIT */
        {
        try {
    ...
    public void disableLocationUpdates() {
		...
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
        if(mtkGeminiSupport){
            disableLocationUpdatesGemini(getDefaultSim());
        } else
        //#endif /* VENDOR_EDIT */
		{
        try {
    ...
    public List<NeighboringCellInfo> getNeighboringCellInfo() {
		...
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
        if(mtkGeminiSupport){
            return getNeighboringCellInfoGemini(getDefaultSim());
        } else
        //#endif /* VENDOR_EDIT */
		{
        try {
    ...
    public int getCurrentPhoneType() {
		...
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for
        if(mtkGeminiSupport){
            return getPhoneTypeGemini(getDefaultSim());
        } else
        //#endif /* VENDOR_EDIT */
		{
        try{
    ...
    public int getCallState() {
        //#ifdef VENDOR_EDIT 
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for mtkGeminiSupport
        if(mtkGeminiSupport){
            return getCallStateGemini(getDefaultSim());
        } else
        //#endif /* VENDOR_EDIT */
		{
        try {
    ...
    public int getDataActivity() {
		...
		//BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for mtkGeminiSupport
			if(mtkGeminiSupport){
				return getITelephony().getDataActivityGemini(getDefaultSim());
			} else 
		//#endif /* VENDOR_EDIT */
			{
    ...
    public int getDataState() {
        ...
        if(mtkGeminiSupport){
            ...
            int dataState = DATA_DISCONNECTED;
            try{
                if (getITelephony().isSimInsert(PhoneConstants.GEMINI_SIM_1)) {                
                    dataState = getDataStateGemini(PhoneConstants.GEMINI_SIM_1);
                    Log.d(TAG, "getDataState is sim1 dataState:"+dataState);
                } 
                if(dataState == DATA_DISCONNECTED && getITelephony().isSimInsert(PhoneConstants.GEMINI_SIM_2)){                
                    dataState = getDataStateGemini(PhoneConstants.GEMINI_SIM_2);
                    Log.d(TAG, "getDataState is sim2 dataState:"+dataState);
                }
                return dataState;
            } catch (RemoteException ex) {
        ...
        try {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {           
                if (events == PhoneStateListener.LISTEN_NONE) {
                    ...
                    //#ifdef VENDOR_EDIT 
                    //BaoZhu.Yu@Prd.CommApp.Telephony, 2012/06/11, Add for mapbarmap failure when only insert 3G on SIM2
                    if((getDefaultSim() == PhoneConstants.GEMINI_SIM_2) && ((events & PhoneStateListener.LISTEN_CELL_LOCATION) !=0 )){
                        mRegistry2.listen(pkgForDebug, listener.callback, events, notifyNow);
                    }
                    //#endif /* VENDOR_EDIT */
                }
