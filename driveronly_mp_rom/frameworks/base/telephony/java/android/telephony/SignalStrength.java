...
//#ifndef VENDOR_EDIT
//BaoZhu.Yu@Prd.CommApp.Telephony, 2013/03/04, Modify for
...
//BaoZhu.Yu@Prd.CommApp.Telephony, 2013/03/04, Modify for 
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == true){
            NetworkType = TelephonyManager.getDefault().getNetworkTypeGemini(mSimId);
        } else {
            NetworkType = TelephonyManager.getDefault().getNetworkType();
        }
        if (DBG) log("Signal before mSimId=" + mSimId + ",NetworkType=" + NetworkType + ",mGsmRscpQdbm=" + mGsmRscpQdbm);
        if(NetworkType < 3 && mGsmRscpQdbm < 0){
            mGsmRscpQdbm = 0;
        }
		...
		if(NetworkType >=3){
		    if(mGsmRscpQdbm < 0){
				mGsmSignalStrength = (mGsmRscpQdbm / 4 + 113) / 2;
				if (DBG) log("Signal mGsmSignalStrength= " + mGsmSignalStrength);
            }
		}
		//////////	
//#endif /* VENDOR_EDIT */		
    ...
    //#ifndef VENDOR_EDIT
    //WangFang@CommApp, 2012/07/23, Modify for change it to OPPO standard
	/*
        try{
	...
	*/
    //#else /* VENDOR_EDIT */
        ...
        log("mSimId =" + mSimId + "getNetworkTypeGemini = " + NetworkType
            +",mGsmRscpQdbm=" + mGsmRscpQdbm
            +",asu=" + asu);
        if(NetworkType >=3){
			int dBm = 0;
            if(mGsmRscpQdbm < 0){
            ...
            else if (dBm >= -88) level = SIGNAL_STRENGTH_HIGHEST;
            else if (dBm >= -92) level = SIGNAL_STRENGTH_GREAT;
            else if (dBm >= -97)  level = SIGNAL_STRENGTH_GOOD;
            else if (dBm >= -102)  level = SIGNAL_STRENGTH_MODERATE;
            else level = SIGNAL_STRENGTH_POOR;    
        }else {
            if (asu < 4 || asu == 99) level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
            else if (asu > 15) level = SIGNAL_STRENGTH_HIGHEST;
            else if (asu >= 13) level = SIGNAL_STRENGTH_GREAT;
            else if (asu >= 10)  level = SIGNAL_STRENGTH_GOOD;
            else if (asu >= 7)  level = SIGNAL_STRENGTH_MODERATE;
            else level = SIGNAL_STRENGTH_POOR;      
        }
    //#endif /* VENDOR_EDIT */
