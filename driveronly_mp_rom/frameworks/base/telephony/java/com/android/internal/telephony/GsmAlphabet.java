...
public class GsmAlphabet {
    ...
    //Tml add for [support Thai using 0x81 to save more character] on 2012.12.17
	...
	public static boolean isEnglish(final String s) {
		int sz = s.length();
	...
	public static boolean isThai(final String s) {
		int sz = s.length();
	...
	public static boolean isRussian(final String s) {
		int sz = s.length();
	...
	public static boolean enableEncodeTo0x81(final String s) {
		boolean ret = false;
	...
	public static byte[] encodeTo0x81(final String src) {
		byte[] b0x81 = null;
		byte[] bytes = null;
		byte base = 0;
		byte data = 0;
		...
		len = src.length();
		b0x81 = new byte[len + 3];

		// flag of 0x81
		b0x81[0] = (byte) 0x81;

		// indicates how many characters in the string
		b0x81[1] = (byte) len;
		...
		for (int i = 0; i < len; i++) {
			...
			try {
				bytes = temp.getBytes("utf-16be");
				//Log.e("Tml",temp+" : "+IccUtils.bytesToHexString(bytes));
			...
			if (!isEnglish(temp)) {
				...
				// base pointer number 
				// indicates bits 15 to 8 of the base  pointer 0xxx xxxx x000 0000
				if (base == 0) {
					base = (byte)(bytes[0] << 1);
					b0x81[2] = base;
				}

				// indicates string data
				data = bytes[1];
	            if((data & 0x80) == 0){
	            	data |= 0x80;
	            }
