...
public class KeyguardSecurityModel {
    ...
    enum SecurityMode {
        ...
        SimPinPuk1, // Unlock by entering a sim pin/puk for sim or gemini sim1.
        SimPinPuk2, // Unlock by entering a sim pin/puk for sim or gemini sim2
    ...
    SecurityMode getSecurityMode() {
        ...
        /// @}
        final IccCardConstants.State simState = updateMonitor.getSimState(PhoneConstants.GEMINI_SIM_1);
        final IccCardConstants.State sim2State = updateMonitor.getSimState(PhoneConstants.GEMINI_SIM_2);
        SecurityMode mode = SecurityMode.None;
        if (simState == IccCardConstants.State.PIN_REQUIRED 
                && !updateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_1, true)
                || (simState == IccCardConstants.State.PUK_REQUIRED 
                && !updateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_1, false))
                /// M:get the remaining puk count of sim card1, if it is 0, set
                /// the unlock mode as unknown.
                && updateMonitor.getRetryPukCount(PhoneConstants.GEMINI_SIM_1) != 0) {
            mode = SecurityMode.SimPinPuk1;
        } else if (sim2State == IccCardConstants.State.PIN_REQUIRED 
                && !updateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_2, true)
                || (sim2State == IccCardConstants.State.PUK_REQUIRED 
                && !updateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_2, false))
                /// M:get the remaining puk count of sim card2, if it is 0, set
                /// the unlock mode as unknown.
                && updateMonitor.getRetryPukCount(PhoneConstants.GEMINI_SIM_2) != 0) {
            mode = SecurityMode.SimPinPuk2;
