...
public final class ViewTreeObserver {
    ...
    void merge(ViewTreeObserver observer) {
        ...
        /// M: merge post draw listeners.
        if (observer.mOnPostDrawListeners != null) {
            if (mOnPostDrawListeners != null) {
                mOnPostDrawListeners.addAll(observer.mOnPostDrawListeners);
            } else {
                mOnPostDrawListeners = observer.mOnPostDrawListeners;
            }
        }
    ...
    /**
     * M: Notifies registered listeners that the drawing pass has finished. If a
     * listener returns true, then indicate the post draw action finished
     * normally. This can be called manually if you want do some thing after
     * ViewRoot finished drawing, such as using it for sync between 2d and 3d
     * drawing thread.
     * 
     * @return True if the post draw action finished normally.
     * @hide
     */
    public final boolean dispatchOnPostDraw() {
        boolean cancelDraw = false;
        final ArrayList<OnPostDrawListener> listeners = mOnPostDrawListeners;
        if (listeners != null) {
            for (OnPostDrawListener listener : listeners) {
                cancelDraw |= !listener.onPostDraw();
            }
        }
        return cancelDraw;
    }
    ...
    /**
     * M: Register a callback to be invoked when the view tree has finished
     * drawing.
     * 
     * @param listener The callback to add
     * @throws IllegalStateException If {@link #isAlive()} returns false
    ...
    public void addOnPostDrawListener(OnPostDrawListener listener) {
        ...
        if (mOnPostDrawListeners == null) {
            mOnPostDrawListeners = new ArrayList<OnPostDrawListener>();
        }

        mOnPostDrawListeners.add(listener);
    }

    /**
     * M: Remove a previously installed post-draw callback.
     * 
     * @param victim The callback to remove
     * @throws IllegalStateException If {@link #isAlive()} returns false
     * @see #addOnPostDrawListener(OnPostDrawListener)
     * @hide
     */
    public void removeOnPostDrawListener(OnPostDrawListener victim) {
        ...
        mOnPostDrawListeners.remove(victim);
    }

    /**
     * M: Interface defined as a callback to be invoked after the view tree has
     * finished drawing.
     * 
     * @hide
     */
    public interface OnPostDrawListener {
        /**
         * Callback method to be invoked when the view tree has finish drawing.
         * 
         * @return
         */
