...
public class BaseInputConnection implements InputConnection {
    ...
    //#ifdef VENDOR_EDIT
    //LiuLei@EXP.GeneralApp.SDK, 2013/07/15, Add for 
	...
	private final static boolean isChinaRegion(){
        String region = android.os.SystemProperties.get("persist.sys.oppo.region", "CN");
    ...
    private final static boolean isThai(char tc){
        return tc >= 0x0e01 && tc <= 0x0e5b;
    }

    @android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
		                      property=android.annotation.OppoHook.OppoRomType.OPPO,
		                      note="(ONLY for Export) recognize Thai vowel or tone")
    private final static boolean isThaiVowelOrTone(char tc) {
        return (tc >= 0x0e47 && tc <= 0x0e4e) || 
                tc == 0x0E31 || 
                (tc >= 0x0e34 && tc <= 0x0e3a);
    }
    ...
    private final void correctCursorPosIfNeed(){
        CharSequence csAfter = getTextAfterCursor(20,0);
        if(csAfter != null && csAfter.length() > 0){
            final int LEN = csAfter.length();
            int cur = 0;
      
            //find the POSITION after cursor which is not vowel Or tone of Thai
            while(cur < LEN) {
                char c = csAfter.charAt(cur);
                if(isThaiVowelOrTone(c)) {
                    cur++;
                    Log.w(TAG,"Thai vowel Or tone after cusor is found! c = " + c);
                } else {
                    break;               
                }                
            }
            
            //move the cursor to the POSITION found above 
            if(cur > 0) {
                commitText("", cur + 1);
            }
        }
    ...
    private final CharSequence removeThaiVowelOrToneIfNeed(CharSequence commitText){
        CharSequence bCs = getTextBeforeCursor(1,0);
         boolean commitVowelOrTone = false;
         //if there is Thai char befor cursor
         if(bCs != null && bCs.length() > 0 && isThai(bCs.charAt(0))){
            commitVowelOrTone = true;
         }

         //if remove Thai vowel or tone in committing text
         if(!commitVowelOrTone && commitText != null && 
            commitText.length()>0 && isThaiVowelOrTone(commitText.charAt(0))){
            commitText = commitText.subSequence(1, commitText.length());
            Log.i(TAG,"The front char is not Thai, removing Thai vowel or tone in committing text!");
         }
    ...
    public boolean commitText(CharSequence text, int newCursorPosition) {
        ...
        //LiuLei@EXP.GeneralApp.SDK, 2013/07/15, Add for 
        if(!isChinaRegion()){
            text = removeThaiVowelOrToneIfNeed(text);
        }
        //#endif /* VENDOR_EDIT */
