...
//#ifdef VENDOR_EDIT
//Shaofeng.Ding@Prd.DesktopApp.Launcher, 2013/04/03, Add for ImageWallpaper 
...
public abstract class WallpaperService extends Service {
    ...
    public class Engine {
        ...
        void updateSurface(boolean forceRelayout, boolean forceReport, boolean redrawNeeded) {
            ...
            //#ifndef VENDOR_EDIT
            //Shaofeng.Ding@Prd.DesktopApp.Launcher, 2013/03/22, Modify for ImageWallpaper
            /*
            ...
            */
            ...
            WallpaperManager wpm = WallpaperManager.getInstance(WallpaperService.this);
            if (null != wpm) {
                wpi = wpm.getWallpaperInfo(); 
            } else {
                Log.v(TAG, "wpm == null");
            }
            if (null != wpi) {
                myWidth = mSurfaceHolder.getRequestedWidth();
                if (myWidth <= 0) myWidth = ViewGroup.LayoutParams.MATCH_PARENT;
                myHeight = mSurfaceHolder.getRequestedHeight();
                if (myHeight <= 0) myHeight = ViewGroup.LayoutParams.MATCH_PARENT;
            } else {
                myWidth = getDesiredMinimumWidth();
                if (myWidth <= 0) myWidth = ViewGroup.LayoutParams.MATCH_PARENT;
                myHeight = getDesiredMinimumHeight();
                if (myHeight <= 0) myHeight = ViewGroup.LayoutParams.MATCH_PARENT;
            }   
            //#endif /* VENDOR_EDIT */
        ...
        void reportVisibility() {
            if (!mDestroyed) {
                ...
                if ((mReportedVisible != visible) || mInitializing) {
                    ...
                    //#ifdef VENDOR_EDIT
                    //Shaofeng.Ding@Prd.DesktopApp.Launcher, 2013/03/07, Add for making sure Surface is created
                    if (DEBUG) Log.v(TAG, "mSurfaceCreated = " + mSurfaceCreated + " mDestroyed = " + mDestroyed);        

                    if (DEBUG && null != mSurfaceHolder.mSurface) {
                        Log.v(TAG, "mSurfaceHolder.mSurface.isValid() = " + mSurfaceHolder.mSurface.isValid());
                    }

                    if (null == mSurfaceHolder.mSurface || !mSurfaceHolder.mSurface.isValid()) {
                        return;
                    }
    ...
    class IWallpaperEngineWrapper extends IWallpaperEngine.Stub
        ...
        public void setDesiredSize(int width, int height) {
            ...
            //#ifdef VENDOR_EDIT
            //Shaofeng.Ding@Prd.DesktopApp.Launcher, 2013/03/22, Add for ImageWallpaper
            mReqWidth = width;
            mReqHeight = height;
            //#endif /* VENDOR_EDIT */
