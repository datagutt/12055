    ...
    */
    ...
    private void setupViewport(boolean updateViewState) {
        ...
        if (mViewportDensityDpi == -1) {
			...
			if (mWebViewClassic.getContentWidth() > 0 && mWebViewClassic.isEmailUsing() && mWebViewClassic.getViewWidth() > 0) {
        		adjust = ((float)(mWebViewClassic.getDisplayViewWidth()))/mWebViewClassic.getContentWidth();
        		if (adjust == 0) {
	...
	/**
     * If current methrod is used by Email application
     *
     * @hide
     */
	@android.annotation.OppoHook(level=android.annotation.OppoHook.OppoHookType.NEW_METHOD,
            ...
            note="XiaoYong.Zhu@Plf.DataApp.Email, Add for improve the email webview display")
	public void updateViewport(boolean isForce) {
        // Update viewport asap to make sure we get correct one.
        setupViewport(isForce);
    }
