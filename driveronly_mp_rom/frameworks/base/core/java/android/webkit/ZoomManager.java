...
class ZoomManager {
    ...
    //#ifndef VENDOR_EDIT
    //Yuping.Yao@Prd.DataApp.Browser, 2012/09/04, Modify for zoom out when the second doubletap
    /*
    ...
    */
    ...
    private class PostScale implements Runnable {
        ...
        public void run() {
//#ifdef VENDOR_EDIT
//JinPeng@Prd.MidWare.View, 2012/12/12, Add for recalculate the minimum zoom scale
            // update mMinZoomScale if the minimum zoom scale is not fixed
            if (!mMinZoomScaleFixed) {
                // when change from narrow screen to wide screen, the new viewWidth
                // can be wider than the old content width. We limit the minimum
                ...
                mMinZoomScale = Math.min(1.0f, (float) mWebView.getViewWidth()
                        / (mWebView.drawHistory() ? mWebView.getHistoryPictureWidth()
                                : mZoomOverviewWidth));
                Log.d(LOGTAG, "PostScale wiewWidth=" + mWebView.getViewWidth() + ", darwHistory="
                    + mWebView.drawHistory() + ",ow=" + mWebView.getHistoryPictureWidth()
                    + ",mZoomOverviewWidth=" + mZoomOverviewWidth + ",mMinZoomScale=" +
                    mMinZoomScale + ",mInitialScale=" + mInitialScale);
                // limit the minZoomScale to the initialScale if it is set
                if (mInitialScale > 0 && mInitialScale < mMinZoomScale) {
                    mMinZoomScale = mInitialScale;
                }
