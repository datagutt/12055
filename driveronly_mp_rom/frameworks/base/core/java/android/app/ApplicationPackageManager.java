...
//#ifdef VENDOR_EDIT
//#@OppoHook
//#luoyaojun@Plf.Framework.SDK add for rom theme.
import android.content.res.OppoResources;
//#endif /* VENDOR_EDIT */
/*package*/
final class ApplicationPackageManager extends PackageManager {
    ...
    //#ifdef VENDOR_EDIT
    //yaoshuang.zhang@Plf.SysSrv.PMS,2012/06/21,add for
        @Override
        public Intent getLaunchIntentForPackage(String packageName, int flags) {
            // First see if the package has an INFO activity; the existence of
            // such an activity is implied to be the desired front-door for the
            // overall package (such as if it has multiple launcher entries).
            Intent intentToResolve = new Intent(Intent.ACTION_MAIN);
            ...
            if (resolveInfo == null) {
                ...
                intentToResolve.addCategory(Intent.CATEGORY_LAUNCHER);
                intentToResolve.setPackage(packageName);
                resolveInfo = resolveActivity(intentToResolve, flags);
            }
            if (resolveInfo == null) {
                return null;
            }
    ...
    public List<PackageInfo> getInstalledPackages(int flags, int userId) {
	    ...
	    //#ifdef VENDOR_EDIT
	    //OppoHook
	    //liumei@Plf.Framework, 2013/07/15, add for filtrating the special app of special market.
	    OppoApplicationPackageManagerHelper.filtratingApp(mContext, mPM, packageInfos);
	    //#endif /* VENDOR_EDIT */ 
	    return packageInfos;          
        } catch (RemoteException e) {
            throw new RuntimeException("Package manager has died", e);
    ...
    @Override public Resources getResourcesForApplication(
        ...
        Resources r = mContext.mMainThread.getTopLevelResources(app.packageName,
                  app.uid == Process.myUid() ? app.sourceDir : app.publicSourceDir,
                          Display.DEFAULT_DISPLAY, null, mContext.mPackageInfo.mCompatibilityInfo.get());
		//#endif /* VENDOR_EDIT */
        if (r != null) {
            return r;
        }
