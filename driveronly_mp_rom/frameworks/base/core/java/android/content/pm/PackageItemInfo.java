...
public class PackageItemInfo {
    ...
    private final static String TAG = "PackageItemInfo";
	  //#endif /* VENDOR_EDIT */
    /**
    ...
    public Drawable loadIcon(PackageManager pm, boolean isConvertEnable) {
        if (icon != 0) {
            ...
            Drawable dr = null;
            if (isConvertEnable) {
                dr = OppoThemeHelper.getDrawable(pm, packageName, icon, getApplicationInfo(), name);
            }               
            if (null == dr || !isConvertEnable) {
                dr = pm.getDrawable(packageName, icon, getApplicationInfo());
                // Log.d("zhangyd","loadIcon()" + icon);
            }
            //#endif /* VENDOR_EDIT */
            if (dr != null) {
                return dr;
