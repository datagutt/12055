...
public class ZygoteInit {
    ...
    private static void preloadResources() {
        ...
        try {
            ...
            if (PRELOAD_RESOURCES) {
                ...
                //#ifdef VENDOR_EDIT
                //#@OppoHook
                //#Jianjun.Dan@Plf.Framework.SDK add for preload resoueces
    ...
    private static void preloadOppoResources(VMRuntime runtime) {
        long startTime = SystemClock.uptimeMillis();
        TypedArray ar = mResources.obtainTypedArray(
                    com.oppo.internal.R.array.oppo_preloaded_drawables);
        int N = preloadDrawables(runtime, ar);
        ar.recycle();
        Log.i(TAG, "...preloaded " + N + " oppo drawable resources in "
                        + (SystemClock.uptimeMillis()-startTime) + "ms.");
        startTime = SystemClock.uptimeMillis();
        ar = mResources.obtainTypedArray(
                        com.oppo.internal.R.array.oppo_preloaded_color_state_lists);
        N = preloadColorStateLists(runtime, ar);
        ar.recycle();
        Log.i(TAG, "...preloaded " + N + " oppo color resources in "
                        + (SystemClock.uptimeMillis()-startTime) + "ms.");
    }
    //endif /* VENDOR_EDIT */
}
