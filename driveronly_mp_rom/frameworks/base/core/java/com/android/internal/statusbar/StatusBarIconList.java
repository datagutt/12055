...
public class StatusBarIconList implements Parcelable {
    ...
    //#ifdef VENDOR_EDIT
    //Baoti.Li@Prd.BasicSrv.StatusBar, 2012/06/12, Add for 
    public void copyN(StatusBarIconList that,int start ,int end) {
        if (that.mSlots == null) {
            this.mSlots = null;
            this.mIcons = null;
        } else {
            ...
            for (int i= start; i<= end; i++) {
                this.mSlots[j] = that.mSlots[i];
                this.mIcons[j++] = that.mIcons[i] != null ? that.mIcons[i].clone() : null;
            }
        }
    }    
    //#endif /* VENDOR_EDIT */
