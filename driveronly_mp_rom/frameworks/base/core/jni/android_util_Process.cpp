...
//Chenps ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/29
jboolean android_os_Process_isProcessSuspend(JNIEnv* env, jobject clazz,
                                      jint pid)
{
    char filename[64];
    ...
    while (fgets(line, sizeof(line), file)) {
        if (sscanf(line, "State: %s", value) == 1) {
            break;
        }
    }
...
static const JNINativeMethod methods[] = {
	...
	//property=android.annotation.OppoHook.OppoRomType.OPPO
	//Chenps ZhiYong.Lin@Plf.Framework modify for BPM 2013/01/29
    {"isProcessSuspend",   "(I)Z", (void*)android_os_Process_isProcessSuspend},
    //#endif /* VENDOR_EDIT */
};
