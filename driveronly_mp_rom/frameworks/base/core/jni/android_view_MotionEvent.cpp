...
static bool validatePointerIndex(JNIEnv* env, jint pointerIndex, size_t pointerCount) {
    if (pointerIndex < 0 || size_t(pointerIndex) >= pointerCount) {
    	ALOGW("MotionEvent jni: validatePointerIndex pointerCount  = %d, pointerIndex = %d",
    			pointerCount, pointerIndex);
	#ifndef VENDOR_EDIT
	//Jiangtao.Guo@Prd.SysSrv.InputManager, 2012/08/14, Remove for no Exception
	/*
		jniThrowException(env, "java/lang/IllegalArgumentException",
...
static bool validateHistoryPos(JNIEnv* env, jint historyPos, size_t historySize) {
    if (historyPos < 0 || size_t(historyPos) >= historySize) {
    	ALOGW("MotionEvent jni: validateHistoryPos historyPos  = %d, historySize = %d",
