...
static void
android_media_MediaScanner_setAudioFileNameRegion(
        JNIEnv *env, jobject thiz, jstring audioFileNameRegion)
{   
    ALOGV("setAudioFileNameRegion");
    MediaScanner *mp = getNativeScanner_l(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, kRunTimeException, "No scanner available");
        return;
    ...
    if (audioFileNameRegion == NULL) {
        jniThrowException(env, kIllegalArgumentException, NULL);
        return;
    }
    const char *audioFileNameRegionStr = env->GetStringUTFChars(audioFileNameRegion, NULL);
    if (audioFileNameRegionStr == NULL) {  // Out of memory
        return;
    }	
    mp->setAudioFileNameRegion(audioFileNameRegionStr);  

    env->ReleaseStringUTFChars(audioFileNameRegion, audioFileNameRegionStr);
}
#endif /*VENDOR_EDIT*/
...
static JNINativeMethod gMethods[] = {
    ...
    //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries, 2013-8-26
    {
        "setAudioFileNameRegion",
        "(Ljava/lang/String;)V",
        (void *)android_media_MediaScanner_setAudioFileNameRegion
    },
    #endif /*VENDOR_EDIT*/
    {
        "extractAlbumArt",
