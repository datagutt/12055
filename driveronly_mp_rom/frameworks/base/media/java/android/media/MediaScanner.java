            ...
            try {
                ...
                if (entry != null && (entry.mLastModifiedChanged || scanAlways)) {
                    if (noMedia) {
                        ...
                        if (isaudio || isvideo) {
                            //#ifdef VENDOR_EDIT
                            //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries. 2013-8-26
                            String fileNameRegion = getFileNameRegion(getFilenameFromPath(path));
                            if (fileNameRegion != null)	
                                setAudioFileNameRegion(fileNameRegion);		
                            Log.w(TAG , "Fengshun yue,getFileNameRegion = " + fileNameRegion);
                            //#endif /* VENDOR_EDIT */
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
    ...
    //#ifdef VENDOR_EDIT
    //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries. 2013-8-26
    public native void setAudioFileNameRegion(String audioFileNameRegion);
    //#endif /* VENDOR_EDIT */
    ...
    //#ifdef VENDOR_EDIT
    //Yugang.Ma@Exp.MultiMedia add for ANSI ID3 support for different countries, 2013-8-26
    private String getFilenameFromPath(String path){
        int filePathIndex = 0;
        int extensionIndex = 0;
        filePathIndex = path.lastIndexOf("/");
        extensionIndex = path.lastIndexOf(".");
        if ((filePathIndex != -1)&&(extensionIndex != -1)&&(filePathIndex < extensionIndex))
        {
            return (path.substring(filePathIndex+1,extensionIndex));
        }
        else
        {
            return null;
    ...
    private String getFileNameRegion(String fileName){
        if (fileName != null)
        {
            String region = SystemProperties.get("persist.sys.oppo.region", "EN");
            if (region.equalsIgnoreCase("ZH") && hasChineseCharacter(fileName))
            { 
                return "zh";			
            }
            else if (region.equalsIgnoreCase("TH") && hasThaiCharacter(fileName))
            {
                return "th";			
            }	
            else if (region.equalsIgnoreCase("RU") && hasRussianCharacter(fileName))
            {
                return "ru";			
            }
            else 
			{
				...
				else if (!region.equalsIgnoreCase("VI") && hasVietNamCharacter(fileName))
				{
					return "vi";
				}			
				else
    ...
    public static final boolean hasChineseCharacter(String chineseStr) {
        ...
        {
            if ((charArray[i] >= 0x4e00) && (charArray[i] <= 0x9fbb)) //chinese:0x4e00--0x9fbb
            { 
                return true;
            }
        }
    ...
    public static final boolean hasThaiCharacter(String thaiStr) {
        char[] charArray = thaiStr.toCharArray();
        for (int i = 0; i < charArray.length; i++) 
        {
            if ((charArray[i] >= 0x0E00) && (charArray[i] <= 0x0E7F)) //thai:0x0E00--0x0E7F
            { 
                return true;
            }
        }
    ...
    public static final boolean hasRussianCharacter(String russianStr) {
        char[] charArray = russianStr.toCharArray();
        for (int i = 0; i < charArray.length; i++) 
        {
            if ((charArray[i] >= 0x0400) && (charArray[i] <= 0x052F)) //Russian:0x4e00--0x052F
            { 
                return true;
            }
        }
    ...
    public static final boolean hasVietNamCharacter(String vietNamStr) {
        char[] charArray = vietNamStr.toCharArray();
        for (int i = 0; i < charArray.length; i++) 
        {
            if (((charArray[i] >= 0x00C0) && (charArray[i] <= 0x00C3)) 
              || ((charArray[i] >= 0x00C8) && (charArray[i] <= 0x00CA)) 		  
              || ((charArray[i] >= 0x00CC) && (charArray[i] <= 0x00CD)) 		  
              || ((charArray[i] >= 0x00D2) && (charArray[i] <= 0x00D5)) 
              || ((charArray[i] >= 0x00D9) && (charArray[i] <= 0x00DA)) 
              || (charArray[i] == 0x00DD) 
              || ((charArray[i] >= 0x00E0) && (charArray[i] <= 0x00E3)) 
              || ((charArray[i] >= 0x00E8) && (charArray[i] <= 0x00EA)) 
              || ((charArray[i] >= 0x00EC) && (charArray[i] <= 0x00ED)) 
              || ((charArray[i] >= 0x00F2) && (charArray[i] <= 0x00F5)) 
              || ((charArray[i] >= 0x00F9) && (charArray[i] <= 0x00FA)) 	
              || (charArray[i] == 0x00FD) 
              || ((charArray[i] >= 0x0168) && (charArray[i] <= 0x0169)) 
              || ((charArray[i] >= 0x01AF) && (charArray[i] <= 0x01B0)) 
              || ((charArray[i] >= 0x01CD) && (charArray[i] <= 0x01CE)) 
              || ((charArray[i] >= 0x1EA0) && (charArray[i] <= 0x1EF9))) //VietNam
            { 
                return true;
            }
        }
