...
public class MediaPlayer
{
    ...
    /**
     * M: add for DRM secure flag @{
     */
    private ISecureWarningPrompt mPrompt = null;
    private Context mContext = null;
    private BroadcastReceiver mWfdReceiver = null;
    private int mSourceType = TYPE_UNKOWN;
    private static final String ACTION_WFD = "com.mediatek.wfd.connection";
    private static final String WFD_CONNECTED = "connected";
    private static final int TYPE_UNKOWN = 0;
    private static final int TYPE_NOT_DRM = 1;
    private static final int TYPE_DRM = 2;
    private static final String ACTION_DRM_PLAY = "com.mediatek.mediaplayer.DRM_PLAY";
    private static final String KEY_IS_PLAYING = "isPlaying";
    private static final String KEY_PLAYER_ID = "playerId";
    private boolean mWfdConnected = false;

    // just enabled when DRM is ongoing.
    private class WfdBroadcastReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            boolean connected = intent.getIntExtra(WFD_CONNECTED, 0) == 1 ? true : false;
            Xlog.v(TAG, "WfdBroadcastReceiver() connected: " + connected);
            mWfdConnected = connected;
            // Show prompt when WFD is connected and DRM source is playing.
            if (connected) {
                mPrompt.show();
            }
        }
    ...
    public  void start() throws IllegalStateException {
         ...
         * M: add for DRM secure flag @{
         */
        Xlog.d(TAG, "start() mSourceType: " + mSourceType);
        // detect DRM security from start to release, which means it will work
        // nomatter it is on pause.
        switch (mSourceType) {
            case TYPE_UNKOWN:
                // if application does not call getMetadata which means we
                // haven't known whether source is of Drm type.
                getMetadata(false, false);
                initSecureFlag();
                break;
            case TYPE_DRM:
                initSecureFlag();
    ...
    private void initSecureFlag() {
        Xlog.d(TAG, "initSecureFlag() mSourceType: " + mSourceType + " mWfdReceiver: "
                + mWfdReceiver);
        if ((mSourceType == TYPE_DRM) && mWfdReceiver == null) {
            try {
                mPrompt = MediatekClassFactory.createInstance(ISecureWarningPrompt.class, mContext);
                mWfdReceiver = new WfdBroadcastReceiver();
                IntentFilter filter = new IntentFilter();
                filter.addAction(ACTION_WFD);
                mContext.registerReceiver(mWfdReceiver, filter);

                // Send broadcast for wfd to handle audio track, because wfd is
                // always running, so we need not to send sticky broadcast.
                Intent intent = new Intent(ACTION_DRM_PLAY);
                intent.putExtra(KEY_IS_PLAYING, true);
                intent.putExtra(KEY_PLAYER_ID, MediaPlayer.this.hashCode());
                mContext.sendBroadcast(intent);
            } catch (Exception e) {
                e.printStackTrace();
                Xlog.e(TAG, "initSecureFlag() exception");
            }
        } else if (mSourceType == TYPE_DRM) {
            // Send broadcast for wfd to handle audio track, because wfd is
            // always running, so we need not to send sticky broadcast.
            Intent intent = new Intent(ACTION_DRM_PLAY);
            intent.putExtra(KEY_IS_PLAYING, true);
    ...
    public void pause() throws IllegalStateException {
         ...
         * M: add for DRM secure flag @{
         */
        Xlog.d(TAG, "pause() mSourceType: " + mSourceType + " mWfdReceiver: " + mWfdReceiver);
        if (mWfdReceiver != null && mWfdConnected && (mSourceType == TYPE_DRM)) {
            // Send broadcast for wfd to handle audio track, because wfd is
            // always running, so we need not to send sticky broadcast.
            Intent intent = new Intent(ACTION_DRM_PLAY);
    ...
    public Metadata getMetadata(final boolean update_only,
         ...
         * M: add for DRM secure flag @{
         */
        boolean isDrm = !data.has(Metadata.DRM_CRIPPLED) || data.getBoolean(Metadata.DRM_CRIPPLED);
        mSourceType = isDrm ? TYPE_DRM : TYPE_NOT_DRM;
        Xlog.v(TAG, "getMetadata() isDrm: " + isDrm);
        /** @} */
        return data;
    }
    ...
    public void release() {
         ...
         * M: add for DRM secure flag @{
         */
        Xlog.d(TAG, "release() mWfdReceiver: " + mWfdReceiver);
        if (mWfdReceiver != null) {
            mContext.unregisterReceiver(mWfdReceiver);
            mWfdReceiver = null;
            mPrompt = null;
            if (mWfdConnected && (mSourceType == TYPE_DRM)) {
                // Send broadcast for wfd to handle audio track, because wfd is
                // always running, so we need not to send sticky broadcast.
                Intent intent = new Intent(ACTION_DRM_PLAY);
                intent.putExtra(KEY_IS_PLAYING, false);
    ...
    public void reset() {
         ...
         * M: add for DRM secure flag @{
         */
        Xlog.d(TAG, "reset() mWfdReceiver: " + mWfdReceiver);
        if (mWfdReceiver != null) {
            mContext.unregisterReceiver(mWfdReceiver);
            mWfdReceiver = null;
            mPrompt = null;
