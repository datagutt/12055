...
// M:
// these native jni function are added for OMA DRM v1.0 implementation.
static jstring android_drm_DrmManagerClient_getContentIdFromRights(
        JNIEnv* env, jobject thiz, jint uniqueId, jobject drmRights) {
    ALOGV("getContentIdFromRights");
    int dataLength = 0;
    char* mData = Utility::getByteArrayValue(env, drmRights, "mData", &dataLength);
    ...
    if (NULL != mData) {
        ...
        result = DrmMtkUtil::getContentId(rights);
        delete mData;
    }
...
static jint android_drm_DrmManagerClient_consume(
        JNIEnv* env, jobject thiz, jint uniqueId, jstring path, int action) {
    ALOGV("consume");
    String8 uri = String8("file://") + Utility::getStringValue(env, path);
    sp<DecryptHandle> handle = getDrmManagerClientImpl(env, thiz)->openDecryptSession(uniqueId, uri.string(), NULL);
    int result = getDrmManagerClientImpl(env, thiz)
            ->consumeRights(uniqueId, handle, action, false);
    getDrmManagerClientImpl(env, thiz)->closeDecryptSession(uniqueId, handle);
    return result;
}
...
static JNINativeMethod nativeMethods[] = {
    ...
    // M:
    // added for OMA DRM v1.0 implementation.
    {"_getContentIdFromRights", "(ILandroid/drm/DrmRights;)Ljava/lang/String;",
                                    (void*)android_drm_DrmManagerClient_getContentIdFromRights},

    {"_consume", "(ILjava/lang/String;I)I",
                                    (void*)android_drm_DrmManagerClient_consume},
