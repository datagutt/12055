...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD ("@@ OMX::CbDisp::CbDisp(0x%08X), tid(%d), WOULD_BLOCK(%d)", this, gettid(), (status == WOULD_BLOCK));
    if (status == WOULD_BLOCK) {
       CallStack cs;
       cs.update();
       cs.dump("~CbDisp");
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD ("@@ OMX::CbDisp(0x%08X)::loop(%d)", this, gettid());
#endif
    for (;;) {
        omx_message msg;
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD ("@@ OMX::CbDisp(0x%08X)::loop(%d) terminated", this, gettid());
#endif
    return false;
}
...
status_t OMX::allocateNode(
    ...
    *node = makeNodeID(instance);
    CallbackDispatcher* pCbDisp = new CallbackDispatcher(instance);
    mDispatchers.add(*node, pCbDisp);
    ALOGD ("OMX(0x%08X)::allocateNode &mDispatchers(0x%08X) added pCbDisp(0x%08X)", this, &mDispatchers, pCbDisp);
