...
#ifdef VENDOR_EDIT 
//Xianfeng.Fu@Prd.Video.VideoCodec,2013/01/23, Add for capture video frame
#ifndef MTK_CMMB_ENABLE
#include "SkImageEncoder.h"
#endif
#endif /* VENDOR_EDIT */
...
status_t AwesomePlayer::setDataSource_l(const sp<MediaExtractor> &extractor) {
	...
	#ifdef VENDOR_EDIT
	//Xianfeng.Fu@Prd.Audio.MediaProvider, 2013/03/08, Add for: 
    if(extractor->countTracks()>=2){
	...
	if (!haveVideo) {
		...
		if (fileMeta != NULL && fileMeta->findInt32(kKeyHasUnsupportVideo, &hasUnsupportVideo)
			&& hasUnsupportVideo != 0) {
			#ifndef VENDOR_EDIT
			//Xianfeng.Fu@MultimediaSrv.VideoCodec,2013/03/13,Add for unsupport video
			notifyListener_l(MEDIA_INFO, MEDIA_INFO_HAS_UNSUPPORT_VIDEO);
			ALOGD("Notify APP that file has unsupportted video");
			#else /*VENDOR_EDIT*/
			ALOGE("Notify APP that file has unsupportted video");
			return UNKNOWN_ERROR;
			#endif /*VENDOR_EDIT*/
		}
...
//Xianfeng.Fu@MultimediaSrv.VideoCodec,2013/03/13,Add for unsupport video
	if (!haveAudio) {
		int32_t hasUnsupportAudio = 0;
		sp<MetaData> fileMeta = extractor->getMetaData();
		if (fileMeta != NULL && fileMeta->findInt32(kKeyHasUnsupportVideo, &hasUnsupportAudio)
			&& hasUnsupportAudio != 0) {
			ALOGE("Notify APP that file has unsupportted audio");
			return UNKNOWN_ERROR;
		}
...
//[4.2migration]TODO: this line is added by Google default 4.2, please check this side effect
    ...
    if (mVideoRenderer != NULL) {
		...
		mVideoBuffer->meta_data()->findPointer(kKeyBufferID, (void**)&header);
		mVideoLastRendered = header->pBuffer + header->nOffset;
		mVideoRenderedLen  = header->nFilledLen;
#endif /* VENDOR_EDIT */
...
void AwesomePlayer::onPrepareAsyncEvent() {
	...
	if((mExtractorFlags & MediaExtractor::MAY_PARSE_TOO_LONG))
	{
		...
		if (mExtractor != NULL) {
			...
			#ifndef VENDOR_EDIT
			//WangHu@Prd.Audio.MediaProvider, 2013/08/15, Remove for 
			//getTrackInfo() need this object in API16.
			mExtractor.clear();
			#endif /* VENDOR_EDIT */
			if (err != OK) {          
				abortPrepare(err);
...
#ifdef VENDOR_EDIT 
//liyajun@MediaSrv.Group.module, 2012/09/21, Add for captureFrame
status_t AwesomePlayer::captureFrame(const String8 & params) { 
	ALOGD("captureFrame(%s)", params.string()); 
	if (mVideoSource == NULL || mVideoLastRendered == NULL) {
	    ALOGE("captureFrame mVideoSoruce = NULL");
	    return UNKNOWN_ERROR;
	}
	...
	int32_t format = 0; 
	void* dst = NULL; 
	int32_t width, height, CropLeft, CropRight, CropTop, CropBottom;; 
	
	//CHECK(mVideoSource->getFormat()->findInt32(kKeyStride, &width));
	//CHECK(mVideoSource->getFormat()->findInt32(kKeySliceHeight, &height));
	sp<MetaData> metaData = mVideoSource->getFormat();
	if (!(metaData->findInt32(kKeyWidth, &width))) {	
	    ALOGE("captureFrame find width error");
	    return UNKNOWN_ERROR;
	}	
	if (!(metaData->findInt32(kKeyHeight, &height))) {	
	    ALOGE("captureFrame find height error");
	    return UNKNOWN_ERROR;
	}
	CropLeft = CropTop = 0;
    CropRight = width - 1;
	...
	void* videoBuf = mVideoLastRendered; 
	uint32_t bufLen = mVideoRenderedLen; 
	mVideoSource->getFormat()->findInt32(kKeyColorFormat, &format); 
	ALOGV("captureFrame:still captureFrame format:%d", format); 
	dst = malloc(2*width*height); 
	if (NULL == dst) { 
		ALOGE("captureFrame failed for mem error"); 
		return UNKNOWN_ERROR; 
	} 

	ColorConverter Converter((OMX_COLOR_FORMATTYPE)format, OMX_COLOR_Format16bitRGB565);//OMX_COLOR_Format32bitARGB8888
	if (true == Converter.isValid()) { 
		//mConverter.convert(width, height, videoBuf, 0, dst, 2*width);// 2*mVideoWidth 
		returnvalue = Converter.convert(
               videoBuf,
               width, height,
               CropLeft, CropTop,
	...
	SkBitmap bmp; 
	bmp.setConfig(SkBitmap::kRGB_565_Config, width, height);//SkBitmap::kARGB_8888_Config
	if (false == bmp.allocPixels()) { 
		ALOGE("captureFrame:Skbitmap failed for setConfig"); 
		free(dst); 
		dst = NULL;
		return UNKNOWN_ERROR; 
	} 
	bmp.setPixels((void*)dst); 

	SkImageEncoder::EncodeFile(params.string(), bmp, SkImageEncoder::kJPEG_Type, 85);//SkImageEncoder::kDefaultQuality 
	ALOGV("captureFrame:JPEG Encode end"); 
