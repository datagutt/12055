...
//    int srcWidth = src.cropWidth();
//    int srcHeight = src.cropHeight();
    unsigned int srcWStride = (src.mWidth + 15) & ~15;
    unsigned int srcHStride = (src.mHeight + 31) & ~31;
	if(mSrcFormat == OMX_COLOR_FormatYUV420Planar){
		srcHStride=src.mHeight;
	 	srcWStride=src.mWidth;
	}    
    DpRect srcRoi;
    srcRoi.x = 0;
    srcRoi.y = 0;
    srcRoi.w = src.mWidth;
    srcRoi.h = src.mHeight;

    ALOGD("src stride aligned, w(%d), h(%d)", srcWStride, srcHStride);

    unsigned int dstWStride = (dst.mWidth + 15) & ~15;
    unsigned int dstHStride = (dst.mHeight + 31) & ~31;
	if(mSrcFormat == OMX_COLOR_FormatYUV420Planar){
		 dstHStride=dst.mHeight;
		 dstWStride=dst.mWidth;
	}
...
*/
	...
	length[2] = length[1];
	ALOGD("Yaddr(%p), Uaddr(%p), Vaddr(%p)", planar[0], planar[1], planar[2]);
