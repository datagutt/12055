...
rinse_repeat:
    ...
    bool bVendorSource = mPlaylist->isVendorSource();
    bool bVendorDiscontinuity = false;
    if (bVendorSource && buffer->size() == 0) {
        bVendorDiscontinuity = true; 
    } else {
    ...
    if (mWasSeeking) {
        ALOGD("first buffer after seek, no need to queue discontinuity");
    ...
    if (bVendorDiscontinuity || !bVendorSource) {
        if (bVendorSource) {
        ...
        if (mPlaylist->isComplete()) {
            ...
            *p = sizeof(int64_t);
            ...
            ALOGD("discontinuity: timeUs_inplaylist = %lld", timeUs_inplaylist);
            p += sizeof(int64_t);
        }
    }
...
void LiveSession::dumpBandwidthItems(Vector<BandwidthItem>* pItems) {
    ...
    for (size_t i = 0; i < pItems->size(); i ++) {
        ALOGD("\tBandwidthItem[%d], %d, %s", i, pItems->itemAt(i).mBandwidth, pItems->itemAt(i).mURI.c_str());
    }
}
