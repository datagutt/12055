...
#ifdef VENDOR_EDIT 
//liyajun@MediaSrv.Group.module, 2012/09/21, Add for
#include<sys/types.h>
#include<sys/stat.h>
#include<fcntl.h>
#endif /* VENDOR_EDIT */
...
status_t MediaRecorder::getduration(int *msec)
{
    ...
    if (mCurrentState & MEDIA_RECORDER_ERROR) {
        ALOGE("getduration called in an invalid state: %d", mCurrentState);
        return INVALID_OPERATION;
    }

     status_t ret = mMediaRecorder->getduration(msec);
