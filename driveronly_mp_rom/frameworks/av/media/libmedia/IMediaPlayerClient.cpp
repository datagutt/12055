...
#ifndef ANDROID_DEFAULT_CODE    	
	 virtual void notify(int msg, int ext1, int ext2,int nPlayerID, const Parcel *obj)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IMediaPlayerClient::getInterfaceDescriptor());
        data.writeInt32(msg);
        data.writeInt32(ext1);
        data.writeInt32(ext2);
		    data.writeInt32(nPlayerID);
        if (obj && obj->dataSize() > 0) {
            data.appendFrom(const_cast<Parcel *>(obj), 0, obj->dataSize());
        }
        remote()->transact(NOTIFYPLAYERID, data, &reply, IBinder::FLAG_ONEWAY);
    }
#endif
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case NOTIFYPLAYERID: {
            ...
            int ext1 = data.readInt32();
            int ext2 = data.readInt32();
			      int nPlayerID = data.readInt32();
			      Parcel obj;
            if (data.dataAvail() > 0) {
                obj.appendFrom(const_cast<Parcel *>(&data), data.dataPosition(), data.dataAvail());
            }
            notify(msg, ext1, ext2,nPlayerID,&obj);
            return NO_ERROR;
        } break;
#endif
        default:
