...
MediaScannerClient::MediaScannerClient()
    :   mNames(NULL),
        ...
        mAudioFileNameRegionEncoding(kAudioFileNameRegionNone)
        #else /* VENDOR_EDIT */
        mLocaleEncoding(kEncodingNone)
        #endif /* VENDOR_EDIT */
{
...
void MediaScannerClient::setLocale(const char* locale)
{
    ...
    //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries 2013/08/26
        ...
        mLocaleEncoding = kEncodingRu;
    else if (!strncmp(locale, "th", 2))
        mLocaleEncoding = kEncodingThai;	
    else if (!strncmp(locale, "vi", 2))
        mLocaleEncoding = kEncodingVi;	
		...
		|| !strncmp(locale, "ms", 2) || !strncmp(locale, "nl", 2)|| !strncmp(locale, "ar", 2) )
	{
...
#ifdef VENDOR_EDIT
//Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries 2013/08/26
void MediaScannerClient::setAudioFileNameRegion(const char* audioFileNameRegion)
{ 
    if (!audioFileNameRegion) return;
	

    if (!strncmp(audioFileNameRegion, "th", 2))
        mAudioFileNameRegionEncoding = kAudioFileNameRegionThai;
    else if (!strncmp(audioFileNameRegion, "ru", 2))
        mAudioFileNameRegionEncoding = kAudioFileNameRegionRu;
    else if (!strncmp(audioFileNameRegion, "vi", 2))
        mAudioFileNameRegionEncoding = kAudioFileNameRegionVi;		
    else if (!strncmp(audioFileNameRegion, "zh", 2)) 
        mAudioFileNameRegionEncoding = kAudioFileNameRegionGBK;	
    else
        mAudioFileNameRegionEncoding = kAudioFileNameRegionNone;
		
    ALOGV("MediaScannerClient -setAudioFileNameRegion audioFileNameRegion:%x \n",mAudioFileNameRegionEncoding); 	
}
#endif /*VENDOR_EDIT*/
...
//			 ALOGD("ch %x \n",ch);
                    #ifdef VENDOR_EDIT
                    //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries 2013/08/26                   
                    uint32_t encode = findPossibleEncodings(ch);
                    if (encode != kEncodingNone)
                        result &= encode;	
                    #else /* VENDOR_EDIT */
    ...
    switch (encoding) {
        ...
        default:
            ...
            if(encoding > 0)
            {
                #ifdef VENDOR_EDIT
                //Yugang.Ma@Exp.MultiMedia add for ID3 support of different countries 2013/08/26
                if (mAudioFileNameRegionEncoding == kAudioFileNameRegionNone)
                {
                property_get("persist.sys.oppo.region",mRegionEncoding,"CN"); 
                mRegionEncoding[2] = '\0';
                if(!strncmp(mRegionEncoding,"OC",2))
                {	
                    if(encoding & 0x2){
                       enc = "gbk";
                    }
                ...
                else if(!strncmp(mRegionEncoding,"RU",2))
                {
                    if (encoding & 0x10) {
                        enc = "windows-1251";
                    }
                ...
                else if(!strncmp(mRegionEncoding,"TH",2))
                {
                    if (encoding & 0x10) {
                        enc = "ISO-8859-11";
                    }
                ...
                else if(!strncmp(mRegionEncoding,"VN",2))
                {
                    if (encoding & 0x10) {
                        enc = "windows-1258";
                    }
                   ...
                   {
                       if (mLocaleEncoding == kEncodingGBK)
                       {
                           enc = "gbk";
                       }
                       else if (mLocaleEncoding == kEncodingRu)
                       {
                           enc = "windows-1251";
                       }
                       else if (mLocaleEncoding == kEncodingThai)
                       {
                           enc = "ISO-8859-11";
                       }
                       else if (mLocaleEncoding == kEncodingVi)
                       {
                           enc = "windows-1258";
                       }	
                ...
                else if (mAudioFileNameRegionEncoding == kAudioFileNameRegionGBK)
                {
                    enc = "gbk";
                }
                else if (mAudioFileNameRegionEncoding == kAudioFileNameRegionRu)
                {
                    enc = "windows-1251";				
                }
                else if (mAudioFileNameRegionEncoding == kAudioFileNameRegionThai)
                {
                    enc = "ISO-8859-11";				
                }	
                else if (mAudioFileNameRegionEncoding == kAudioFileNameRegionVi)
                {
                    enc = "windows-1258";				
                }
                #else /* VENDOR_EDIT */
