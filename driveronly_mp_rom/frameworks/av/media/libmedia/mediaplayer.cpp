...
status_t MediaPlayer::getDuration_l(int *msec)
{
    ...
    if (mPlayer != 0 && isValidState) {
        ...
        ret = mPlayer->getDuration(&mDuration);
        if(mDuration < 0){        
           SXLOGD("getDuration:%d < 0 ",mDuration);
           mDuration = 0;
        }
#endif
        if (msec)
...
void MediaPlayer::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    if ((listener != 0) && send) {
#ifndef ANDROID_DEFAULT_CODE
        if(!(notifyThreadId == getThreadId()))
              Mutex::Autolock _l(mNotifyLock);	
        notifyThreadId = getThreadId();
#else
        Mutex::Autolock _l(mNotifyLock);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
void MediaPlayer::notify(int msg,int ext1,int ext2,int nPlayerID, const Parcel *obj)//by Changqing
{ 
   if (MEDIA_SET_PLAYERID == msg)
   	{
   	   mPlayerID = nPlayerID;
	     ALOGD("mPlayerID = %d",mPlayerID);
   	}
     
   else
   	{
   	  if (mPlayerID == nPlayerID)
   	    notify(msg,ext1,ext2,obj);
      else
        ALOGD("Notify object changed!");
   	}
}
#endif
