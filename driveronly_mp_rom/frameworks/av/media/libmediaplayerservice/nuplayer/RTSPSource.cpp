...
bool NuPlayer::RTSPSource::isSeekable() {
    //TODO: to judge live streaming here
    return true;
}
...
void NuPlayer::RTSPSource::onMessageReceived(const sp<AMessage> &msg) {
	...
	if (mIsServerPlaying) {
	    notifyAsyncDone(msg, NuPlayer::Source::kWhatPlayDone);
	    mPlayAsResume = false;
	}
    ...
    switch (what) {
        case MyHandler::kWhatConnected:
            onConnected();
#ifndef ANDROID_DEFAULT_CODE
	    notifyAsyncDone(msg, NuPlayer::Source::kWhatConnDone);
	    (new AMessage(kWhatBufferingUpdate, mReflector->id()))->post();
//	    notifyStartBuffering(true);
#endif
            break;
	    ...
	    (new AMessage(kWhatBufferingUpdate, mReflector->id()))->post();
	    notifyAsyncDone(msg, NuPlayer::Source::kWhatSeekDone);
#endif
            break;
        }
	...
	// mtk80902: ALPS00434921
        ...
        {
	    notifyAsyncDone(msg, NuPlayer::Source::kWhatPauseDone);
	    mIsServerPlaying = false;
	    break;
        }
#endif
...
void NuPlayer::RTSPSource::notifyAsyncDone(const sp<AMessage>& msg, uint32_t notif) {
    sp<AMessage> notify = mNotify->dup();
