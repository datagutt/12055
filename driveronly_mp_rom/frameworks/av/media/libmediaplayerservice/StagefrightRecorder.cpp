...
#ifdef VENDOR_EDIT 
//GuoLe@Prd.Video.VideoCodec, 2012/06/19, Add for new op setCameraParameters for recorder
status_t StagefrightRecorder::setCameraParameters(const String8 &params)
{
//    LOGV("setCameraParameters");
    status_t ret = BAD_VALUE; // UNKNOWN_ERROR
...
#ifdef VENDOR_EDIT 
//Xianfeng.Fu@Multimedia, 2012/01/23, add for only one Recorder can work
    status_t errAudioSource = mAudioSourceNode->initCheck();
    if (errAudioSource != OK) {
...
#ifdef VENDOR_EDIT
//GuoLe@Prd.Video.VideoCodec, 2012/06/05, Add for  "getduration"
status_t StagefrightRecorder::getduration(int *msec)
{
//    LOGD("StagefrightRecorder::getduration");

    if (mWriter != 0) {
        return mWriter->getduration(msec);
    } else {
        *msec = 0;
    }
