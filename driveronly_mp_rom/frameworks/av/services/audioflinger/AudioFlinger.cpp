...
status_t AudioFlinger::getActiveAudioPids(Vector<int32_t>& outPids)
{
    ...
    for (size_t i = 0; i < mAudioSessionRefs.size(); i++) {
        AudioSessionRef *r = mAudioSessionRefs[i];
        for (int j = 0; j < mActiveSessionIds.size(); j++) {
...
//            ioHandle, keys.string(), gettid(), IPCThreadState::self()->getCallingPid());
	...
	// Liangchangwei @Plf.MediaSrv	2013/08/09, for add new parameter for get pid
	#ifdef VENDOR_EDIT		  
    ALOGV("getParameters(): io %d, keyvalue %s, tid %d, calling pid %d",
            ioHandle, keys.string(), gettid(), IPCThreadState::self()->getCallingPid());
	if(keys == "get_pid")
	{
		...
		ALOGV("mAudioSessionRefs.size() %d",mAudioSessionRefs.size());
		ALOGV("mActiveSessionIds.size() %d",mActiveSessionIds.size());
		for (size_t i = 0; i < mAudioSessionRefs.size(); i++) {
			AudioSessionRef *r = mAudioSessionRefs[i];
			for (int j = 0; j < mActiveSessionIds.size(); j++) {
				if (r->mSessionid == mActiveSessionIds[j]) {
					...
					ALOGV(" i = %d j %d r->mPid %d",i,j,r->mPid);
					
					char testbuff[100];
					sprintf(testbuff,"%d",r->mPid);
					//out_pid = out_pid + " " + r->mPid ;
					String8 teststr = String8(":");
					teststr = teststr + testbuff;
					ALOGV("testbuff %s teststr %s",testbuff,teststr.string());
					out_pid += teststr;
				}
			}
		} 
		ALOGV("out_pid %s",out_pid.string());
        return out_pid;
	}
	#endif
...
#ifdef VENDOR_EDIT
//Jirui.kang@BasicDrv.Audio, 2013/02/21, Add for switch afe when streamtype have changed
...
#ifdef VENDOR_EDIT
#ifdef EXTERNAL_CODEC_SUPPORT
//Jirui.kang@BasicDrv.Audio, 2013/02/17, Add for external codec 
        static int gStreamType = -1;
    #if 0 //2013/02/17 modify for switch afe when streamtype have changed
        if((gStreamType != (int)track->mStreamType))
    #else
        if((gStreamType != (int)track->mStreamType) && !haveMusic)
    #endif
        {
            gStreamType = (int)track->mStreamType;
            AudioParameter param = AudioParameter();
            param.addInt(String8(AudioParameter::keySetStreamType), (int)track->mStreamType);
            mOutput->stream->common.set_parameters(&mOutput->stream->common, param.toString());

            ALOGD("external codec test------mActiveTracks[%d]-----mStreamType=%d",i,track->mStreamType);
        }
    #if 1 //2013/02/17 Add for switch afe when streamtype have changed
        if(AUDIO_STREAM_MUSIC == track->mStreamType) {
            haveMusic = true;
        }
    #endif
...
#endif // DOLBY_AUDIOFLINGER_AUDIODUMP
#endif  //VENDOR_EDIT
...
//liangchagnwei@Plf.Mediasrv, add for dolby
        ...
        if (dsEffect != 0){
            ...
            for (unsigned int i=0;i<mActiveTracks.size();i++){
                ...
                if (track->isEffectBypassStreamType()) {
					...
					ALOGV("track->isEffectBypassStreamType() ");
    ...
    {
        ALOGD("DOLBY_AUDIOFLINGER_AUDIO_DUMP: Open the wav file for PCM dump");
        pFileAudioDump = fopen("/data/mixerOutput.wav", "wb");
                                                    ...
                                                    int triggerSession)
{
    ...
    if (thread != 0) {
        ...
        /* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
		#ifdef OPPO_WOLFSON_CODEC_SUPPORT
            char StreamTypeMusic[30];
            
            if(3 == mStreamType) /* Music Stream Type:3 */
            {
                sprintf(StreamTypeMusic, "SetStreamTypeMusic=%d", 1);
                String8 Key = String8(StreamTypeMusic);
                //ALOGV("StreamTypeMusic=%s", StreamTypeMusic);
                mTrackOutput->stream->common.set_parameters(&mTrackOutput->stream->common, Key);
            }
		#endif
        #endif /* VENDOR_EDIT */
...
void AudioFlinger::PlaybackThread::Track::stop()
{
    ...
    if (thread != 0) {
        ...
        /* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
            ...
            if(3 == mStreamType) /* Music Stream Type:3 */
            {
                sprintf(StreamTypeMusic, "SetStreamTypeMusic=%d", 0);
                String8 Key = String8(StreamTypeMusic);
                //ALOGV("StreamTypeMusic=%s", StreamTypeMusic);
                mTrackOutput->stream->common.set_parameters(&mTrackOutput->stream->common, Key);
            }
...
#ifdef VENDOR_EDIT
#if defined(DOLBY_DAP_OPENSLES_PREGAIN)
      , mDsLeftVolume(UINT_MAX), mDsRightVolume(UINT_MAX),mBypassed(false)
