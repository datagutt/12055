...
public:
    ...
    int GetMonthDays(int iSolarYear, int iSolarMonth);
    
    // 判断是否为闰年
    bool IsLeapYear(int iSolarYear) const;
...
inline bool SolarDate::IsLeapYear(int iSolarYear) const
{
	return (!(iSolarYear % 4) && (iSolarYear % 100)) || !(iSolarYear % 400);
}
...
{
    /*size_t*/int days = sm_yearDays[m_month - 1] + m_day; //luoyaojun
