...
const effect_descriptor_t gDolbyAudioDescriptor = {
        ...
        {0x4721c740, 0xfe36, 0x11e0, 0xa358, {0x00, 0x02, 0xa5, 0xd5, 0xc5, 0x1b}}, // uuid Eq NXP
...
status_t DolbyAudioClient::process(short *in, short *out, int n_samples)
{
    ...
    if(n_samples%DM3_MIN_PROCESS_SAMPLES){
        MY_LOOP_LOGW("The nummber of sample is not the multiple of  DM3_MIN_PROCESS_SAMPLES or channel is not stereo");
...
status_t DolbyAudioClient::paramProcessSwitch(int event, int* value, int vLength)
{
    ...
    switch(event)
    {
        ...
        case TDAS_HFE:
		{
            ALOGV("case TDAS_HFE value1:%d, value2:%d, vLenght:%d", value1, value2, vLength);
            result=setHFELevel(value1);		
        }break;
        case TDAS_NB:
        { 
            //ALOGV("case TDAS_NB value1:%d, value2:%d, vLenght:%d", value1, value2, vLength);
			//result=SetSoundEffectParameter(TDAS_NB,value1,value2);
        ...
        case TDAS_MSR:
        {
            //ALOGV("case TDAS_MSR value1:%d, value2:%d, vLenght:%d", value1, value2, vLength);
			//result=SetSoundEffectParameter(TDAS_MSR,value1,value2);
        ...
        case SET_ENABLE_TYPE:
        {
            ALOGV("case SET_ENABLE_TYPE value1:%d, value2:%d, vLenght:%d", value1, value2, vLength);
                ALOGV("Enable effect");
                if(2 == vLength)
                {
                    SoundEffectEnable(value1, value2);
                }
                else
                {
                    ALOGV("Enable effect para error!");
                }
        }
        break;
        case TDAS_SLC:
        {
            ALOGV("case TDAS_SLC value1:%d, value2:%d, vLenght:%d", value1, value2, vLength);
            if(value1 == SLC_LEVEL)
            {
                ALOGV("Set TDAS_SLC value ==>SLC_LEVEL");
                SetSoundEffectParameter(TDAS_SLC, SLC_LEVEL,value2);
            }
            else if(value1 == SLC_DEPTH)
            {
                ALOGV("Set TDAS_SLC value ==>SLC_DEPTH");
                SetSoundEffectParameter(TDAS_SLC, SLC_DEPTH,value2);
            }
        }
        break;
        default:
...
status_t DolbyAudioClient::SetSoundEffectParameter(int se, int parm, int value){
    ALOGD("SetSoundEffectParameter(se:%d,parm:%d,value:%d)",se,parm,value);
    if (NULL == mTdasHandle){
        ALOGE("Dolby not open");
        return UNKNOWN_ERROR;
    }
...
status_t DolbyAudioClient::updateSetting()
{
    ...
    ALOGV("updateSetting: mCurrentGEQType:%d, mCurrentPlayMode:%d ",mCurrentGEQType,mCurrentPlayMode);
