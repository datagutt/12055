...
#ifdef VENDOR_EDIT
//Fangfang.Hui@Prd.PlatSrv.OTA, 2013/04/08, Add for can not install after an error installer
        ensure_path_unmounted("/system");
        ensure_path_unmounted("/data");
        ensure_path_unmounted("/cache");//if we got an error, unmount all the devices we may had mounted
#endif /* VENDOR_EDIT */
        return INSTALL_ERROR;
    }
...
#ifndef VENDOR_EDIT
/*Saturn.Luo@ProDrv.pm modify for modify sdcard path*/
        if (ensure_path_mounted("/sdcard") != 0) {
#else
        if (ensure_path_mounted("/storage/sdcard0") != 0) {
#endif /*VENDOR_EDIT*/
            LOGE("Can't mount %s\n", path);
