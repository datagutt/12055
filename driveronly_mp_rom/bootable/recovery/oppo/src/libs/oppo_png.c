...
static pthread_mutex_t apng_load_mutex = PTHREAD_MUTEX_INITIALIZER;
...
byte apng_load(PNGCANVAS * pngcanvas,char* imgname) {
  ...
  pthread_mutex_lock(&apng_load_mutex);
  
  if (imgname[0]=='@'){
    char * icotheme_name = imgname;
    icotheme_name++;
    snprintf(zpath, 255, "%s/icons/%s.png",OPPO_DIR,icotheme_name);
  }
  else
    snprintf(zpath, 255, "%s/%s.png",OPPO_DIR,imgname);
...
exit:
  ...
  free(data_png.data);
  pthread_mutex_unlock(&apng_load_mutex);
  return result;
}
...
byte apng_loadfont(PNGFONTS * pngfont,const char* imgname) {
  ...
  //-- LOAD DATA FROM ZIP
  char zpath[256];
  snprintf(zpath, sizeof(zpath)-1, "%s/%s.png",OPPO_DIR,imgname);
  AZMEM data_png;
  if (!az_readmem(&data_png,zpath,1)) return 0;
...
static pthread_mutex_t apng_stretch_mutex = PTHREAD_MUTEX_INITIALIZER;
...
byte apng_stretch(
  ...
  pthread_mutex_lock(&apng_stretch_mutex);
  ...
  pthread_mutex_unlock(&apng_stretch_mutex);
  return 1;
}
