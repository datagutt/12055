...
/* Get Input Event */
byte aipGetInput(struct input_event *ev, int dont_wait){
  if (_aip==NULL){
    return 0;
  }
  int r;
  int n;
  do {
    r = poll(_aip->fds, _aip->n, dont_wait?dont_wait:-1);
    if(r>0){
      for(n=0;n<_aip->n;n++) {
