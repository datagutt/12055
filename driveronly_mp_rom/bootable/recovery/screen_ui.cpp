...
#ifdef VENDOR_EDIT
//Fangfang.Hui@Prd.PlatSrv.OTA, 2012/06/11, Add for scroll the line when the string is too long
static enum FocusScrollState {
    FOCUSSTATE_TYPE_INIT,
    FOCUSSTATE_TYPE_NOSCROLL,
    FOCUSSTATE_TYPE_SCROLL
} gFocusScrollState = FOCUSSTATE_TYPE_INIT;

static int cur_point = 0, focus_line_length = 0;
static const int SCROLL_STEP = 4;   
#endif /* VENDOR_EDIT */
...
void ScreenRecoveryUI::draw_screen_locked()
{
    ...
    if (show_text) {
        ...
        if (show_menu) {
			#ifndef VENDOR_EDIT
            //Fangfang.Hui@Prd.PlatSrv.OTA, 2012/06/11, Modify for recovery font color
...
#ifdef VENDOR_EDIT
//Fangfang.Hui@Prd.PlatSrv.OTA, 2012/06/11, Add for scroll the line when the string is too long
        if (show_text && show_menu && (gFocusScrollState == FOCUSSTATE_TYPE_SCROLL)){
            cur_point = cur_point - SCROLL_STEP;
            if (cur_point + focus_line_length < 5){
                cur_point = 20;
            }
            //if the focus line need to scroll, we should refresh the screen
...
#ifndef VENDOR_EDIT
//Fangfang.Hui@Prd.PlatSrv.OTA, 2013/03/21, Remove for no need text
/*
...
void ScreenRecoveryUI::StartMenu(const char* const * headers, const char* const * items,
    ...
    if (text_rows > 0 && text_cols > 0) {
        ...
        for (; i < text_rows; ++i) {
			...
			#ifndef VENDOR_EDIT
    		//Fangfang.Hui@Prd.PlatSrv.OTA, 2012/06/11, Modify for scroll the line when the string is too long
	            strncpy(menu[i], items[i-menu_top], text_cols-1);
	            menu[i][text_cols-1] = '\0';
			#else /* VENDOR_EDIT */
				strncpy(menu[i], items[i-menu_top], kMaxCols-1);
	            menu[i][kMaxCols-1] = '\0';
			#endif /* VENDOR_EDIT */
        }
