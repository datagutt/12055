...
int try_mount(const char* device, const char* mount_point, const char* fs_type, const char* fs_options) {
    if (device == NULL || mount_point == NULL || fs_type == NULL)
        return -1;
    int ret = 0;
/* OPPO 2013-02-04 jizhengkang azx Add begin for display chinese in sdcard */
#ifdef SUPPORT_UTF8
	if (!strcmp(fs_type, "vfat")) {
		ret = mount(device, mount_point, fs_type, MS_NOATIME | MS_NODEV | MS_NODIRATIME, "utf8=1");
	} else
#endif	
/* OPPO 2013-02-04 jizhengkang azx Add end */
    if (fs_options == NULL) {
...
int ensure_path_mounted(const char* path) {
    LOGI("ensure_path_mounted: path[%s]\n", path);
...
int ensure_path_mounted_at_mount_point(const char* path, const char* mount_point) {
    ...
    if (strcmp(v->fs_type, "yaffs2") == 0) {
        ...
        if ((result = try_mount(v->device2, mount_point, v->fs_type2, v->fs_options2)) == 0)
            return 0;
/* OPPO 2013-01-23 jizhengkang azx Add begin for reason */
#ifdef NTFS_SUPPORT
		if (strcmp(v->fs_type, "vfat") == 0) {
			char mount_cmd[PATH_MAX];
			sprintf(mount_cmd, "/sbin/ntfs-3g %s %s", v->device, mount_point);
			LOGE("run command: %s\n", mount_cmd);
...
#endif
/* OPPO 2013-01-23 jizhengkang azx Add end */
        return result;
    } else {
