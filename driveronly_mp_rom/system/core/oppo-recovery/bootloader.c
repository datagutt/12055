...
#include <sys/stat.h>
#include <unistd.h>
/* OPPO 2013-01-09 jizhengkang Add begin for reason */
#include <sys/types.h>
#include <fcntl.h>
/* OPPO 2013-01-09 jizhengkang Add end */
...
static void wait_for_device(const char* fn) {
    ...
    do {
        ...
        ret = stat(fn, &buf);
        if (ret) {
            printf("stat %s try %d: %s %s\n", fn, tries, strerror(errno), fn);
            sleep(1);
        }
    } while (ret && tries < 10);
    if (ret) {
...
/* OPPO 2013-01-09 jizhengkang Modify begin for use read instead of fread */
#if 0
...
#endif
/* OPPO 2013-01-09 jizhengkang Modify end */

/* OPPO 2013-01-09 jizhengkang Modify begin for use write instead of fwrite */
#if 0
...
static int set_bootloader_message_block(const struct bootloader_message *in,
    ...
    dev = open(v->device, O_WRONLY | O_SYNC);
    if (dev < 0)  {
			LOGE("Can't open %s\n(%s)\n", v->device, strerror(errno));
        return -1;
    }
