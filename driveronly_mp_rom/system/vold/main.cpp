...
#ifdef VENDOR_EDIT
 SLOGE("has defined VENDOR_EDIT");
#endif
#ifdef MTK_2SDCARD_SWAP
 SLOGE("has defined MTK_2SDCARD_SWAP");
#endif
...
#else
	//No eMMC(internal SD-Card), this may be the nand project
...
static int process_config(VolumeManager *vm) {
    ...
    */
        ...
        if (!(fp = fopen("/etc/oppo.vold.fstab.dynamic", "r"))) {
			SLOGE("Open config file fail (oppo.vold.fstab.dynamic), Try to open (vold.fstab) instead");
				if (!(fp = fopen("/etc/vold.fstab", "r"))) {
					SLOGE("Open config file fail (vold.fstab)");
					return -1;
				}
            }
       #else /*OPPO_EMMC_DYNAMICMOUNT_SUPPORT*/
       if (!(fp = fopen("/etc/vold.fstab", "r"))) {
		    SLOGE("Open config file fail (vold.fstab)");
            return -1;
            }
       #endif /*OPPO_EMMC_DYNAMICMOUNT_SUPPORT*/
